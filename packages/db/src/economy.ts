...
type TxnKind = 'salario' | 'premio' | 'staff' | 'patrocinador' | 'otro'
...
/** Nómina semanal: paga el salario del contrato a los corredores humanos (SPEC 9). */
export async function runPayroll(tx: Tx, worldId: string, gameDay: number): Promise<void> {
  if (gameDay % 7 !== 0) return
  const rows = await tx
    .select({ riderId: contracts.riderId, salary: contracts.salary })
    .from(contracts)
