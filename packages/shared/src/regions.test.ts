...
describe('shared: modelo de regiones (confederaciones UCI)', () => {
  ...
  it('asigna continentes conocidos correctamente', () => {
    ...
    expect(continentForCountry('KZ')).toBe('Asia') // Kazajistán en Asia (ACC)
    expect(continentForCountry('TR')).toBe('Europe') // Turquía en Europa (UEC)
    ...
    expect(continentForCountry('GU')).toBe('Oceania') // Guam en Oceanía (OCC)
  })
