...
const SEALED_RESULTS: Record<string, string> = {
  /**
   * RESELLADO EN LA v39, y las cuatro huellas cambian porque han cambiado tres LEYES, no una
   * calibración. Ver docs/balance.md «v39»; lo que hay que justificar aquí es que la FORMA de cada
   * etapa sigue siendo la que tiene que ser, y de hecho es mejor.
   *
   * Lo que se movió, en orden de peso:
   *
   * - **El sprint tiene submotor propio** (§12.7): cada hombre ABRE el sprint a una distancia de la
   *   línea y hay dos maneras de equivocarse —pasarse de lo que aguanta, o dejar que el primero se
   *   vaya más de una ventana—. Toca todas las llegadas agrupadas, o sea las dos llanas.
   * - **El remate ya no es gratis.** El régimen de sprint imponía la velocidad de los últimos
   *   kilómetros y el coste se seguía cobrando con el compromiso del grupo: el pelotón cruzaba la
   *   meta a 59,9 km/h con el compromiso en 0,10 y el trabajo de todos —lanzadores incluidos—
   *   valía CERO. Ahora se cobra por la velocidad que de verdad se lleva.
   * - **El exponente del coste del ritmo lo manda la ley** (`rhythmCostExponent`): 2,56 en llano y
   *   1,0 en cuesta, el inverso del exponente con el que la potencia se convierte en velocidad, en
   *   vez de un 1,6 fijo para las dos cosas. Anclado en el ritmo de un día normal, así que cambia
   *   la curvatura y no el nivel. Toca la etapa entera, en los dos escenarios.
   *
   * Y la forma resultante:
   *
   *   llana-180-0  gana spr-6  ·  173 juntos, 2 a +127 s, 1 a +468 s
   *   llana-180-1  gana spr-0  ·  172 juntos, y 1, 1 y 2 desperdigados hasta +336 s
   *   reina-150-0  gana pel-67 ·  11 relojes, 8 delante, y el grueso a +477/+592/+615 s
   *   reina-150-1  gana gc-0   ·  8 relojes, GANADOR EN SOLITARIO a +92 s del segundo grupo
   *
   * Las dos LLANAS siguen metiendo al pelotón entero en un solo reloj —173 y 172 de 176— con una
   * cola de dos o tres rezagados, que es de lo que va una llana; y en las dos gana un SPRINTER,
   * con `spr-0` repitiendo victoria respecto de la v38. Las dos REINAS reparten el campo en once y
   * ocho relojes con el grueso a más de siete minutos, que es de lo que va una reina. Y la segunda
   * hace algo que la v38 no hacía: **un jefe de filas gana SOLO**, con noventa y dos segundos sobre
   * el grupo de detrás. Es exactamente lo que se buscaba al subir el tempo del puerto —«en una
   * etapa reina falta que los campeones se esfuercen un poquito más»— y no se puede pedir mejor
   * señal de que la montaña volvió a decidir.
   *
   * Ninguna banda de calibración se queda fuera: la batería sale 1327/1327 y la campaña de 200
   * corridas, 32 de 33 objetivos (docs/balance.md «v39»).
   */
