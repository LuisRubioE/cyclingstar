...
/**
 * RESELLADO EN LA v56, y lo que hay que justificar es DÓNDE se mueve y dónde no.
 *
 * La v56 prohíbe que dos grupos se atraviesen: si un perseguidor alcanza a una fuga, se ponen
 * juntos, en vez de cruzarse los relojes y seguir siendo dos grupos (el dueño lo vio en la etapa 16
 * del Giro: «iban 3 en cabeza y un grupo de 16 detrás, y de repente es al revés»). Eso solo puede
 * cambiar una etapa donde de verdad se alcancen grupos, y eso es exactamente lo que sale:
 *
 * - **LAS DOS LLANAS NO SE MUEVEN NI UN SEGUNDO**, ni un puesto. En el llano la fuga la caza el
 *   pelotón —regla que ya existía y no se toca— y no hay grupos cruzándose por el camino.
 * - **`reina-150-0` cambia de ganador**, y cambia a mejor: ganaba `pel-71`, uno de doce hombres que
 *   llegaban al MISMO reloj (14760), y ahora gana `bar-5` en solitario a 14799. Un cazaetapas que
 *   aguanta delante en vez de que el grupo que le alcanza le pase de largo y le deje descolgado.
 * - **`reina-150-1` conserva ganador y tiempo** (`gc-0`, 14525) y pasa de 48 a 46 relojes distintos:
 *   dos grupos que antes se cruzaban ahora llegan juntos.
 *
 * O sea: cero movimiento donde no hay grupos que se alcancen, y en la montaña el cambio va en la
 * dirección de la regla. Cualquier otra cosa que mueva esta huella hay que volver a justificarla.
 */
