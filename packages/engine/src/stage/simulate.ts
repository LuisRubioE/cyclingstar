...
interface RiderSim {
  ...
  pulling: boolean
  /**
   * PARA QUÉ TIRA (v47), lo que el dueño pidió con todas sus letras: «busca de algún modo dejar una
   * evidencia que explique por qué o para qué tira cada ciclista de un grupo». `null` cuando no
   * tira. Es observación como `pulling`: se escribe donde el turno ya se decidió y nadie la lee de
   * vuelta. Ver `PullMotive` para en qué se diferencia de la función `pullReason` (v13).
   */
  pullMotive: PullMotive | null
  /** …y PARA QUIÉN tira (v57): el id del hombre por el que da la cara, o null si no es por nadie. */
  pullFor: string | null
  /**
   * EL PARTE DEL DÍA (v47). Contadores de observación: se escriben donde el motor ya hacía la cosa
   * que cuentan y NADIE los lee de vuelta, así que no pueden cambiar una carrera. Ver `StageEffort`.
   */
  parte: StageEffort
}

/** El parte en blanco con el que arranca cada corredor. */
function parteEnBlanco(): StageEffort {
  return {
    kmAlFrente: 0,
    kmEnFuga: 0,
    kmDescolgado: 0,
    ...
    cerillos: 0,
    reservaGastadaS: 0,
    gasto: { rodar: 0, relevo: 0, reserva: 0, cerillos: 0, banderas: 0 },
    pajaraKm: null,
    descuelgueKm: null,
  }
}
  ...
  const scored = members.map((m) => {
    ...
    return {
      ...
      duty:
         ...
         *
         * El dueño, en la etapa 19: «otra vez el maillot amarillo tirando… bueno, no es el pelotón,
         * es un grupo de 20 del que solo tiran 10». Y el arreglo de la v50 —que le quitó el trabajo
         * en el PELOTÓN— no llegaba aquí, porque fuera del pelotón el motor cambia de listón: en una
         * fuga o en un grupeto relevan todos (`relayDutyThresholdLoose` = 0) y el rol deja de pesar,
         * así que el líder de la carrera, con su 0,1 de rol y la frescura por encima, salía positivo
         * y entraba al turno como uno más. Medido en el Giro del banco: en un grupo de 14 tiraban 6,
         * y entre ellos un `lider`.
         *
         * Y es que el listón suelto dice una verdad a medias. «En una fuga colabora todo el mundo»
         * vale para los que van a ganar algo yendo deprisa; el que lleva el maillot ya va ganando, y
         * lo que hace en carretera es esconderse y obligar a los demás a mover la carrera. Por eso
         * esto es SOLO para el que va primero: el segundo, el tercero y el cuarto sí tienen que dar
         * la cara, que es justo lo que el dueño echaba en falta en la etapa 13 («ellos quieren luchar
         * por la carrera… ¡y curiosamente no veo que lo hagan!»).
         *
         * No es un veto: es el orden. Queda el último de la fila del deber, así que el SUELO de
         * relevistas (`relayMinPullers`) lo saca al frente cuando de verdad no queda nadie —va solo,
         * o los que le acompañan están peor que él—, que es la doctrina de su propio rol: «solo tira
         * si no queda nadie más». Y en un ABANICO no se aplica, por lo mismo que el «no puedo ganar»
         * tampoco: en una fila con el viento de lado detrás del último no hay rueda, hay cuneta.
         */
        (enAbanico || m.input.gcRank !== 1 ? 0 : STAGE.relayRaceLeaderPenalty) +
...
export function simulateStage(entrada: StageInput, seed: string, probe?: StageProbe): StageOutput {
   ...
   */
  /**
   * DÓNDE EMPIEZA LA BAJADA EN LA QUE VAMOS (v57). La selección del descenso se tira por BLOQUE, y
   * un bloque son cien metros: una bajada de cinco kilómetros son cincuenta tiradas por corredor y
   * eso no descuelga, aniquila. Ver `STAGE.descentSelectKm`.
   */
  let descentStartKm = 0
  let tipoBloquePrevio: string | null = null
  ...
  for (let i = 0; i < n; i++) {
      ...
      if (teamPlans.size > 0) {
        ...
        if (kmRestantes >= STAGE.helpBackMinKmToGo) {
          ...
          for (const plan of teamPlans.values()) {
            ...
            /**
             * …Y CUANDO NO BAJA NADIE, SE DICE POR QUÉ (v57).
             *
             * El dueño, viendo caerse a su líder en el km 196: «el líder se queda atrás… ¿y nadie de
             * su equipo tira para ayudarle?». La regla existe y funciona, pero cuando NO salta se
             * callaba, y el silencio es indistinguible del abandono: el lector no puede saber si el
             * equipo le dejó tirado o si sencillamente no quedaba nadie que pudiera bajar.
             *
             * Así que el caso de «nadie baja» tiene ahora su propio aviso, con el motivo concreto:
             * o no queda un solo compañero en condiciones de bajar, o los que quedan son los únicos
             * que sostienen la carrera del equipo por delante, o la etapa ya no da para rescates.
             * Es la misma doctrina que el motivo del relevo: si el motor decide algo, la crónica
             * tiene que poder enseñar en qué se basó.
             */
            if (cuantos === 0) {
              if (!sinAyudaAvisado.has(leaderId)) {
                sinAyudaAvisado.add(leaderId)
                const enCarrera = plan.memberIds
                  .filter((id) => id !== leaderId && !rebels.has(id))
                  .map((id) => sims.get(id))
                  .filter(
                    (m): m is RiderSim =>
                      m != null && m.finishTs === null && m.abandonedKm === null,
                  )
                const porque =
                  enCarrera.length === 0
                    ? 'sin_equipo'
                    : disponibles.length === 0
                      ? 'nadie_puede'
                      : 'todos_guardan'
                log.emit(km, suGrupo.tS, 'ayuda_al_jefe', 'no_help_for_leader', [leaderId], {
                  jefeId: leaderId,
                  porque,
                  // Cuántos de los suyos siguen en carrera y cuántos de ésos podrían bajar: con los
                  // dos números el lector ve la diferencia entre «le abandonaron» y «no había nadie».
                  suyos: enCarrera.length,
                  podian: disponibles.length,
                  gapS: Math.round(gap),
    ...
    const advance = (
      ...
      /**
       * El reloj del GRUPO PRINCIPAL en este bloque (v47): con él se sabe si este grupo va por
       * delante del grueso de la carrera o por detrás, que es lo que necesitan el parte del corredor
       * y el motivo del relevo. Viaja como argumento y no se busca aquí dentro a propósito —se
       ...
       *
       * NO se hace porque **rompe justo lo que viene a arreglar**: con el tope puesto, el jefe al
       * que bajan a buscar volvía el 66 % de las veces contra el 70 % del que se quedaba solo (en
       * llano, 68 % contra 82 %), o sea que la ayuda le PERJUDICABA. Sin el tope, 81 % contra 63 %.
       * Un grupeto rueda al ritmo de sus hombres fuertes y los demás van a rueda: es el modelo que
       * el motor ya tenía para todos los grupetos, y el rescate no es una excepción.
       *
       * …Y EL SEGUNDO MOTIVO SE HA CAÍDO EN LA v47, así que se retira en vez de dejarlo mintiendo.
       * Decía «no hace falta: `shatter` no se ejecuta sobre los grupos de descolgados, así que un
       * grupeto NUNCA suelta a nadie —el jefe no puede quedarse atrás de su propio rescate—», y esa
       * frase era la descripción de un DEFECTO (ver la criba de los grupos de descolgados, abajo)
       * que ya no está: hoy un grupeto sí se parte y el jefe sí puede perder la rueda de los suyos.
       * Vuelto a medir con la criba puesta, sobre el mismo escenario del banco y 24 semillas: **con
       * su equipo vuelve al grupo 8 de 24 veces y pierde 181 s de media; sin equipo, 0 de 24 y 382
       * s**. La ayuda sigue ayudando, y ahora además puede fallar, que es lo que hace en carretera.
       */
      ...
      /** Ver `m.pullMotive`: las mismas ramas de `relayTurn`, en el mismo orden. */
      /**
       * …Y PARA QUIÉN (v57). El dueño, leyendo la radio: «ahora dice por qué tiran, pero dice algo
       * así como *his team's card for this finish*… **pero no dice quién es, wey**». Y tiene razón:
       * un motivo que nombra a un hombre sin nombrarlo no es una evidencia, es un acertijo.
       *
       * El id sale de la MISMA fuente que decidió el motivo, no de una búsqueda aparte: del tren, el
       * jefe al que se lanza; de los tres motivos de equipo, el hombre del plan por el que ese
       * equipo gasta —el jefe de filas para el maillot y la general, la carta del día para la etapa—.
       * Los motivos que no son por nadie (solo, abanico, fuga, grupeto, rol) devuelven null, y la
       * pantalla no inventa un nombre donde no lo hay.
       */
      ...
      for (const m of members) {
        ...
        m.pulling = pulling
        /**
         * …Y PARA QUÉ TIRA (v47). El dueño lo pidió al ver a un equipo dando relevos en el TERCER
         * grupo mientras su líder iba en el segundo: «¿para qué carajos tiran si en ese grupo donde
         * están no está su líder? … busca de algún modo dejar una evidencia que explique por qué o
         * para qué tira cada ciclista de un grupo».
         *
         * Se lee de las MISMAS ramas con las que `relayTurn` acaba de decidir el turno, en el mismo
         * bloque y en el mismo orden en que allí mandan. No es una etiqueta puesta a posteriori: si
         * el motivo que sale no cuadra con lo que se ve en la carretera, lo que está mal es la
         * decisión —que es exactamente lo que una evidencia tiene que poder enseñar—.
         */
        if (!pulling) {
          m.pullMotive = null
          m.pullFor = null
        } else {
          const porQue = motivoDelRelevo(m)
          m.pullMotive = porQue.motivo
          m.pullFor = porQue.para
        }
        ...
        m.work += cost
        /**
         * …Y SE APUNTA EN EL PARTE (v47), separando el peaje de rodar de lo que cuesta DAR LA CARA.
         * El reparto no es una estimación: `blockCost` con la misma llamada y `pulling` a false es
         * literalmente lo que este hombre habría gastado a rueda en este mismo bloque, así que lo
         * que sobra ES el viento que dio.
         *
         * La segunda evaluación SOLO se hace para el que tira, y no es tacañería: son veinte hombres
         * de ciento setenta y seis, y este bucle corre para cada corredor en cada uno de los mil
         * ochocientos bloques de una etapa. Preguntarlo también para el que va a rueda costaría
         * trescientas mil llamadas por etapa para obtener una resta que vale cero por definición
         * —el precedente está tres pantallas más arriba, en el régimen de remate de la v39, que se
         * fue de 1.950 s a 2.665 s de batería por dos evaluaciones de más—.
         */
        if (!pulling) m.parte.gasto.rodar += cost
        else {
          const aRueda =
            blockCost(block, compromisoReal, false, relayers.size, STAGE.dx, arropo) *
            (1 + STAGE.heatCostScale * calor)
          m.parte.gasto.rodar += Math.min(cost, aRueda)
          m.parte.gasto.relevo += Math.max(0, cost - aRueda)
          m.parte.kmAlFrente += STAGE.dx
        }
        // Dónde iba: por delante del grueso de la carrera, o por detrás. El grupo principal no es
        // el que se llama `peloton` sino el que lleva la gente (v29), que es la misma pregunta que
        // se hace la radio y el mismo `mainId` con el que el motor mide los boquetes.
        if (group.id !== (mainId ?? PELOTON)) {
          if (group.tS < relojPrincipal) m.parte.kmEnFuga += STAGE.dx
          else m.parte.kmDescolgado += STAGE.dx
        }
      }
      return next
    }
    ...
    const dropOut = (m: RiderSim, group: Group, delayS = 0): void => {
      ...
      const near = m.hurt
        ...
        : shed.find(
            (sg) =>
              // …Y NUNCA AL GRUPO DEL QUE ACABA DE SALIR (v47). Desde que la criba mira también a
              // los grupos de descolgados, el grupo de origen puede ser uno de estos, y sin este
              // filtro el hombre que se suelta volvería a entrar en él en el mismo bloque —un
              // descuelgue que no descuelga—. Hoy no llegaría a pasar por los números
              // (`driftDropGapSeconds` 20 s contra `grupetoJoinGapSeconds` 12 s), y justo por eso
              // se dice aquí: que la criba funcione no puede depender de que dos constantes que
              // nadie ató sigan ordenadas como están.
              sg.id !== group.id &&
              Math.abs(sg.tS - tS) <= STAGE.grupetoJoinGapSeconds &&
    ...
    const comesOff = (
      ...
      if (
        ...
        m.parte.cerillos += 1
        m.parte.gasto.cerillos += STAGE.matchCost
        m.driftS = 0
        return false
      }
      return true
    ...
    const shatter = (group: Group, members: RiderSim[], paceFraction: number): string[] => {
      ...
      for (const m of members) {
        ...
        // El km en que se le vació el depósito, para el parte del corredor (v47). Es el primero:
        // el que se arrastra veinte kilómetros a cero reventó al principio de esos veinte.
        m.parte.pajaraKm ??= km
        dropOut(m, group)
      ...
      /**
       * LA BAJADA SELECCIONA AL PRINCIPIO, NO KILÓMETRO TRAS KILÓMETRO (v57).
       *
       * El dueño, sobre la etapa 17 del Giro: «¿qué chingados pasó entre el km 191 y el 192?».
       * Medido: entre el km 191,1 y el 196,1 el pelotón pasó de **164 a 78 corredores** en cinco
       * kilómetros de bajada al −6,1 %, con la energía media al 38 % y **cero corredores vacíos**.
       * Nadie se quedó por no poder: se quedaron por un dado.
       *
       * Y el dado es correcto —en una bajada se pierde la rueda por un error, no por reventar— pero
       * se tiraba EN CADA BLOQUE DE CIEN METROS. Cinco kilómetros son cincuenta tiradas por
       * corredor, y cincuenta tiradas de una probabilidad pequeña son una probabilidad enorme:
       * `dropDescentFactor` = 0,08 está puesto como «el más suave de los tres» y en una tirada
       * suelta lo es, pero nadie había mirado qué hace ACUMULADO a lo largo de una bajada entera.
       * Con esto, además, la bajada se comía media parrilla y el motor se la devolvía kilómetros
       * después (78 → 108): el ciclo «estalla y se rehace» contra el que este mismo motor avisa.
       *
       * Se pierde la rueda AL PRINCIPIO, que es cuando el ritmo sube de golpe y se abren los huecos;
       * pasado eso el grupo se ha ordenado y baja junto. Así la selección de una bajada deja de
      ...
      if (block.tipo === 'subida') {
        ...
        for (const m of alive) {
          ...
          if (m.reserveS > 0) {
            ...
            m.energy = Math.max(0, m.energy - cost)
            m.work += cost
            m.parte.reservaGastadaS += spend
            m.parte.gasto.reserva += cost
            continue
          }
          m.driftS += drift
    ...
    /**
     * …Y UN GRUPO DE DESCOLGADOS TAMBIÉN SE PARTE (v47). Esto no existía: `shatter` se llamaba sobre
     * el pelotón y sobre los movimientos, y NUNCA sobre un `shed-N`. O sea que un grupo, en cuanto
     * nacía de un descuelgue, quedaba SELLADO: podía subir un puerto entero al 10 % sin perder un
     * solo hombre, porque el motor no le pasaba la criba por encima.
     *
     * No es una esquina rara: es lo que se ve en cuanto el pelotón se parte pronto. Medido en la
     * etapa 9 del Giro —184 km, final en alto de 12,8 km al 5,9 % con los últimos 2,8 al 9,7 %—, el
     * pelotón hacía SU trabajo y se deshacía de 99 a 8 hombres… mientras los dos grupos de detrás
     * subían el puerto congelados: `shed-17` cruzaba la meta con los MISMOS 67 corredores que tenía
     * veinte kilómetros antes, y `shed-27` no solo no perdía a nadie sino que ENGORDABA de 37 a 94
     * absorbiendo todo lo que el pelotón soltaba. De ahí salía la queja del dueño con todas sus
     * letras: «es una llegada en alto con un puerto brutal al final… y el que llega en el puesto 150
     * solo perdió 26 segundos».
     *
     * Y por eso el arreglo no trae física nueva: la criba de la subida (la deriva de la v26) ya
     * estaba escrita y calibrada, y lo único que hacía falta era dejarla mirar donde no miraba. Un
     * grupo se rompe en un puerto por lo que llevan sus hombres en las piernas, no por la etiqueta
     * con la que el motor lo bautizó al nacer —es la misma corrección que la v33 hizo en el plan de
     * equipo («en el pelotón» es el grupo que lleva la gente, no el que conserva el id)—.
     *
    ...
    const attemptFrom = (source: Group, kind: MoveKind, target: Group | null): void => {
      ...
      if (gap < STAGE.tacticJumpMinGapSeconds) {
        for (const r of party) {
          ...
          s2.work += c
          // Cuenta como intento en el parte: el cerillo se gastó igual, que es lo que pasa cuando
          // uno se tira y no sale. Si no, el día del que ataca ocho veces sin éxito saldría vacío.
          s2.parte.cerillos += 1
          s2.parte.gasto.cerillos += c
          if (r.riderId === instigator.riderId) s2.parte.ataques += 1
          else s2.parte.saltos += 1
        }
        /**
      ...
      for (const r of party) {
        ...
        s.work += cost
        s.parte.cerillos += 1
        s.parte.gasto.cerillos += cost
        if (r.riderId === instigator.riderId) s.parte.ataques += 1
        else s.parte.saltos += 1
    ...
    if (shed.length > 0) {
      ...
      for (const sg of [...shed].sort((a, b) => a.tS - b.tS)) {
         ...
         */
        /**
         * …PERO «MENOS RELOJ» NO ES «HA VUELTO» (v57). Esta línea preguntaba solo por el SIGNO del
         * hueco: cualquier grupo cuyo reloj fuera por delante del pelotón se lo tragaba el pelotón,
         * estuviera a un segundo o a dos minutos. Y por delante del pelotón no solo van los que
         * vuelven: van TODOS los que se han caído de una fuga —`dropOut` los mete en `shed` con el
         * reloj de la fuga, que es el de un grupo que va camino arriba—, y también los grupos que la
         * criba deja delante en la carretera.
         *
         * El dueño lo vio en la etapa 17 del Race Italy: «hay 10 escapados que sacan 59 segundos a un
         * grupo de 45, y 56 segundos más tarde un grupo de 90… y en el km 192 los tres grupos se han
         * juntado. WTF». Los tres estaban por delante del reloj del pelotón, así que el pelotón se
         * los tragó a todos de golpe: dos minutos de carrera borrados en un kilómetro.
         *
         * Medido mecánicamente sobre un Giro entero —cada kilómetro, cada grupo que desaparece dentro
         * de otro— salían **42 fusiones con más de 30 s de hueco**, con casos de 97 s y de 113 s.
         *
         * Alcanzar es LLEGAR, no ir por delante: el pelotón absorbe cuando su reloj se pone a la
         * altura del otro, y eso pasa dentro de la puerta de siempre (`regroupGapSeconds`). Los
         * bloques son de 100 m, así que un reloj no salta esa puerta de un bloque al siguiente: el
...
function disputeBanner(
  ...
  for (const c of contenders) {
    c.energy = Math.max(0, c.energy - STAGE.bannerCost)
    c.parte.gasto.banderas += STAGE.bannerCost
  }
