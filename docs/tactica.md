...
> adversarios** —contra el código, por cobertura contra `diseno/catalogo-situaciones.md`, contra el dueño
> contra `diseno/mapa-requisitos-duenio.md`, y por coste y riesgo contra `diseno/mapa-bancos.md`— que sacaron
> **86 fallos**. **Los 86 llevan corrección escrita en el texto, y este recuento es el de la
> auditoría final, que los volvió a mirar uno a uno contra el documento de HOY en vez de fiarse de los
> informes de corrección**: **71 aplicados enteros** y **15 aplicados en parte**, con la mitad
> desestimada y su porqué en el **Apéndice C**, que recoge **14 objeciones** con nombre de revisor
> —una de ellas, la del maillot sin equipo (R04.5), cubre **dos** fallos, y por eso son quince
> aplicados en parte y catorce filas—. **Cero sin tocar.** La versión anterior de esta cabecera daba
> **5 por no aplicados** porque no aparecían en ningún informe de corrección; la auditoría los
> encontró cerrados y dice por qué, que es lo que faltaba: son **segundas lecturas del mismo defecto**
> por dos refutadores distintos, y un defecto se arregla una vez, no dos. Los seis defectos que más de
> un refutador sacó por separado —y donde por tanto viven los cinco— son el **coste del bloque contra
> la Frontera 2** (§9.1bis y el punto único de aplicación), la **línea base de CI** (§7.7, medida en el
> paso 0), la **numeración de invariantes** (§7.6 como fuente única, 47-73), **R04.5** (fila 8 del
> Apéndice C más el invariante 73), el **suelo de `mountain.top10GapSeconds`** ([DUEÑO 22]) y la banda
> de **`medianLeadGroupRiders`** ([DUEÑO 19]). Lo único que la auditoría tuvo que cerrar escribiendo
> fue el último resto de la contradicción de `StageOutput`: la casilla del **paso 2** de §8 seguía
> diciendo «`race?` y `memory?` en `StageInput`/`StageOutput`» cuando §1.2 y §9.6 ya habían resuelto
> que **`StageOutput` no gana ni un campo** (Frontera 3). Dos pares de refutaciones se contradicen
> entre sí; las dos caras van escritas en el Apéndice C y ninguna se resuelve eligiendo bando.

> **Las veintitrés decisiones de §10 están CONTESTADAS.** El dueño respondió el **10-09-2026**:
> «acepto todas las recomendaciones» —las **trece de clase A** y las **diez de clase B**, que eran las
> que tocaban una banda, un umbral o un corte que había puesto él en persona—. Lo que esa respuesta
> **echa a andar** no se queda en §10: está propagado en **§9**, y su índice es **§9.0**, con la
> decisión, la consecuencia y el sitio exacto donde está escrita. Diez cosas que estaban paradas
> esperándole ahora corren.
...
Este documento **sustituye al `docs/tactica.md` anterior**, el que el dueño llamó superficial, y
ocupa su sitio. Es el diseño único que se implementa: recorre los
**28 racimos** de `diseno/catalogo-situaciones.md` —las 494 situaciones agrupadas por la pieza de motor que
...
Sale de cuatro propuestas (`diseno/tactica-propuesta-incremental.md`, `-agentes.md`, `-dominio.md`,
...
- Lo que decide el dueño va marcado **[DECISIÓN DEL DUEÑO n]** en el texto y recogido en §10 con
  ...
  cosa. **Las veintitrés están contestadas el 10-09-2026, todas aceptadas**, así que hoy ninguna marca
  de ésas apunta a un hueco: apunta a una respuesta. El defecto se conserva escrito porque es lo que
  se habría hecho de no contestar, y esa diferencia es la que §9.0 registra.
...
> diseño hermano (`entrenamiento.md` §0), que ya está cerrado. Este documento comparte con él
...
| **Cómo nace un corredor**            | `generateNpcRider`, `sampleNpcAge`, Banister                                                                                     | Parcela de `entrenamiento.md`, cerrada.                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
3. Los bancos que hoy no tienen general (`realQueens`, `calendarQueens`, `climbs`) siguen corriendo
   igual mientras no se decida darles una. **Y ya está decidido: [DECISIÓN DEL DUEÑO 16], aceptada el
   10-09-2026 — general sintética SÍ a `realQueens`, NO a `calendarQueens` ni a `climbs`.** (La marca
   de esta línea decía **[DECISIÓN DEL DUEÑO 5]**, que es la de sustituir el escenario canónico y no
   la de las generales; queda corregida aquí para que el puntero no siga apuntando mal.) La regla de
   compatibilidad sigue siendo la que permite que los otros dos corran con déficit 0 para siempre.
...
- **Si la banda tenía ancla de dominio, lo que se discute es la regla.** Y aquí hay que corregir un
  ...
  tabla `diseno/mapa-requisitos-duenio.md` §13, contadas ahí y no de memoria**, y van las trece por nombre
  ...
  1. `flat.breakawayWinPct` 5-16
  2. `smallTours.flatMoveWorstMarginS` 0-900
  3. `timeTrials.tailPct` 8-15
  4. `calendarQueens` fuga en montaña 6-30
  5. **`mountain.top10GapSeconds` suelo 40**
  6. la cola de la reina del CLI
  7. el PAV ≥ 69 del ganador de pavé
  8. la saturación white-roads ≤ 0,96
  9. **las tres bandas provisionales de la v33 en UNA sola fila** (llano 2-10 «luego 5-16» —que es la
     misma banda de la fila 1 y **no cuenta dos veces**—, **montaña 24-45** con la vuelta a 25 en la
     v34, y pájaras de Lombardía ≤ 12)
  10. el listón de bots con 5★
  11. la alarma de cracks
  12. **rueda contra cara en llano** (5 % → «¿vemos con un 10 %?», v38; sale 14 % con el exponente)
      —**la fila que este documento se dejaba fuera**, y por eso su lista enumeraba catorce cosas
      mientras el recuento decía trece—
  13. la ventana de ataques de la general
  ...
  **Consecuencia práctica y no menor: tres bandas que este documento proponía mover dentro de §9.1
  ...
  dueño y por tanto salen de §9.1 y entran en §10 como decisiones numeradas.** **Y las tres están
  contestadas el 10-09-2026, con un sí** (decisiones 22, 23 y 6): el suelo de `top10GapSeconds` se
  queda en 40 y su techo sube a 360, `mountain.breakawayWinPct` se re-ancla y se publica sin banda, y
  la cola de la reina sube su techo al 16 %. Lo que esto demuestra es lo que la regla perseguía: **las
  tres se movieron con su firma delante y ninguna por silencio**.
...
Esta tabla es el diff del rediseño en una hoja. La columna «hoy» sale de `diseno/mapa-simulate-decisiones.md`
§5, verbatim.
...
R03.4  QUIÉN ES ELEGIBLE PARA IRSE
       (a) veto al velocista puro       — ya existe (breakawaySkipSprThreshold 70)       (S-473)
           ...
           ACEPTADA el 10-09-2026: el paso 6 LLEVA la pieza, no mezcla sin ella, y el veto
           vuelve a disparar en PRS (SPR maduro ≈ 61) y en CON (≈ 54).
...
no lo mide nadie**: `diseno/mapa-bancos.md` §7.2 lo dice explícitamente («ningún banco cruza `protagonistas`
...
**Frontera con `entrenamiento.md`, que ya está cerrado.** Ese documento se queda con la mitad
...
R08.4  EL QUE LLEGA SIN RITMO Y EL QUE VIENE DE UNA VUELTA               (S-468, S-385, S-482)
       ...
       La recuperación desigual (S-482) es de `entrenamiento.md` §4: aquí solo se LEE,
...
R10.4  EL OBJETIVO DE CARRERA DEL JUGADOR                                (S-022, S-028)
       ...
       el pico de forma que `entrenamiento.md` §5 ya sabe programar. El mánager lo reparte
...
**Y esa decisión ya está tomada: S-467 NO está abierta, está CERRADA POR DECISIÓN, y con un no.** El
dueño contestó la decisión 15 el **10-09-2026** aceptando la recomendación —**la ley en cuesta no se
toca**—, así que la vía de la ley queda cerrada con firma y no en suspenso. Lo que sigue en pie es
todo lo de arriba: se ataca **solo** por sus causas (R03.4d, R18.1, R18.3, R18.4, R28) y se mide en el
paso 21 con `breakClimberShare` y `calendarQueens.breakawayWinPct`. La distinción importa y hay que
sostenerla en todo el documento: **«abierta» y «cerrada con un no» se leen igual desde lejos y no son
lo mismo** —una espera respuesta, la otra la tiene—. **La única que sigue abierta en esta tanda es
S-207**, y eso no cambia.
...
único paso de este documento que toca la LEY DE VELOCIDAD**, por eso está separado y va el último y
**SOLO** [DECISIÓN DEL DUEÑO 7]. **ACEPTADA el 10-09-2026: el paso 20 SE HACE y deja de ser
opcional**, así que las siete constantes de este racimo cuentan en firme en el Apéndice B y el
invariante 43 se re-ancla ahí con causa escrita y predicción declarada. El «si se recorta, se recorta
esto» que decía esta casilla **ya no rige**: era el contrafactual de cuando la decisión 7 esperaba
respuesta, y se conserva solo como eso —de recortarse, se habría recortado esto y el diseño no
quedaba cojo, porque las otras once filas de R14 no tocan `targetSpeed`—.
...
R17.1  LOS ROLES SALEN DEL FINAL REAL, NO DE LA ETIQUETA           (S-047, S-049, S-052)
       ...
       → y toca «el 70 % del campo acaba de gregario», que es el punto 14 de §14 del mapa. HIPÓTESIS,
         ...
         ACEPTADA el 10-09-2026, y aquí aceptar coincide con no mover nada — pero la diferencia
         va dicha: sigue SIN BANDA y sigue fuera de los criterios de «hecho», ahora POR DECISIÓN
         y no por defecto. El 45-65 % espera a que la pregunta del dueño se conteste.
...
R17.5  EL GRUPO DE CABEZA DE UNA REINA                    (S-367, LA DEUDA SIN BANDA de la v23)
       ...
       **[DECISIÓN DEL DUEÑO 19]**, ACEPTADA el 10-09-2026: 5-15 en valle y sin banda en alto,
       las dos mitades. Y el motivo
...
| Estadística             | Qué cuenta                                                                            | Banco                                                                                          | Banda                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ----------------------- | ------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `medianLeadGroupRiders` | existe (`smallTours.ShapeStats`, ventana 30 s), **mide 1**, es la deuda de la v23     | `smallTours` ShapeStats **+ `realQueens` partida por `queenFinalKind`** (portada en el paso 0) | **5-15 en reinas de final NO alto**; **sin banda en final en alto** (v26 §5). **[DUEÑO 19] ACEPTADA el 10-09-2026: gana banda, y la ausencia de banda en final en alto también es la decisión**                                                                                                                                                                                                                                                                                                                                                    |
...
| `gregarioSharePct`      | fracción del campo con rol gregario                                                   | `smallTours`                                                                                   | **SE PUBLICA SIN BANDA** en los pasos 0 y 8 (hoy 70 %). El 45-65 % que este documento traía como banda va a §10 como **[DECISIÓN DEL DUEÑO 21]** con recomendación, porque `diseno/mapa-requisitos-duenio.md` §14 punto 14 deja el reparto de roles como **pregunta abierta** —«otra pregunta… este banco la deja a la vista sin contestarla» (v48 §4)— y ponerle banda es contestarla por la puerta de atrás. No es criterio de «hecho» del paso 8. **[DUEÑO 21] ACEPTADA el 10-09-2026: sigue sin banda, y ahora por decisión y no por defecto** |
...
`mentality`, `contestSprints/Climbs`, `targetRiderId` o rol por decisión externa» (`diseno/mapa-bancos.md`
...
R24.1  CALIDAD DE DIRECCIÓN                                              (S-009, S-014)
       ...
       doctrina que `entrenamiento.md` aplica al entrenador bot.
...
**Cómo se mide, y el banco NO puede ser `world`.** `world.ts` **no simula etapas** —`diseno/mapa-bancos.md`
...
R26.1  LA READMISIÓN MIRA EL NÚMERO                                    (S-494, S-464)
       ...
       → por primera vez el grupeto TIENE ALGO QUE PERDER, y por eso se organiza. Es la causa que
         ...
         ACEPTADA el 10-09-2026: `readmitBlockRiders` 20 ENTRA, en el paso 10. El corte de tiempo
         mira el número, y las cinco situaciones que sin esto se cerraban «por fuera» —S-310,
         S-359, S-365, S-371 y S-412— se cierran CON la pieza.
...
R26.5  EL GRUPETO QUE YA NO ES LA CARRERA SÍ SE CRIBA           (S-443, deuda §14.2)
       ...
       ACEPTADA el 10-09-2026: el techo pasa a 16 % (`grandTour.queenLastGroupPct` 8-16 y
       `realQueens.lastGroupPct` 7-16), la criba se mezcla ENTERA y el paso 10 ya no se para
       ahí: es la banda nueva la que vigila.
...
**Y cinco de esas quince se cierran DE VERDAD desde el 10-09-2026, no «por fuera».** S-310, S-359,
S-365, S-371 y S-412 dependían de que el corte mirase el número: sin `readmitBlockRiders` el grupeto
no tiene nada que perder y las cinco quedaban cerradas por la forma, no por la causa. La decisión 8
está **aceptada**, la constante entra en el paso 10 y la causa existe: el grupeto se organiza por
miedo, y el miedo es proporcional a lo poco que sois.

**Y una deuda que este racimo NO cierra, y que hay que nombrar porque es la más pesada de la lista
abierta.** `diseno/mapa-requisitos-duenio.md` §14.1, primer punto por peso: **«los que pierden en montaña 5
...
R28.2  DÓNDE CAE LA ÚLTIMA CIMA                                  (S-486, CONTRARIO nº 3)
       ...
       → hoy las CINCO reinas de Race Alps dejan 22, 1, 31, 50 y 19 km tras la última cota. Y aquí
         ...
         dice literalmente lo contrario: `diseno/mapa-requisitos-duenio.md` §3, fila v42 §3 / v43 §11
...
R28.7  ALTITUD                                                                   (S-479)
       ...
       consta en §9.2 y en §7.6 — **pero una excepción de OTRO rango que la del paso 20, y desde que
       [DUEÑO 7] está aceptada (10-09-2026) hay que decirlo separado**: el 18d es excepción a «nadie
       edita `physics.ts`» y **tiene que dejar el invariante 43 EN BANDA**; el paso 20 es el único
       que **mueve la ley**, y por eso C2 se lee «vigila todas las tandas tácticas menos el paso 20».
       La redacción anterior —«C2 pasa a decir pasos 18 y 20, no solo el paso 20»— metía las dos
       excepciones en el mismo saco y dejaba sin saber qué se comprueba aquí: se comprueba el 43,
       y tiene que salir intacto.
...
R20—. Verificado ID a ID contra las listas `**Situaciones**:` de `diseno/catalogo-situaciones.md`:
...
| **S-207** reagrupamiento en el descenso                 | `PARCIAL`   | sí   | **No se cierra**: es la deuda §14.1 de `diseno/mapa-requisitos-duenio.md` («los que pierden 5 minutos se reintegran demasiado fácil»), refutada por el lado de `rejoinGapSeconds`. Se **mide sin banda** en los pasos 0 y 12 (R26) para no empeorarla a ciegas. |
...
abierta y se mide sin banda. La de no regresión es **S-129**, que además del `CUBIERTO` gana
invariante propio (el **73**).

**Y S-207 es ya la única abierta del documento, porque S-467 dejó de serlo.** La versión anterior de
este párrafo decía «como S-467, más abajo», y eso valía mientras la decisión 15 esperaba respuesta.
**Contestó el 10-09-2026 y aceptó**: la ley en cuesta **no** se toca, así que **S-467 está cerrada por
decisión, y con un no** —se ataca por sus causas y se mide en el paso 21—. S-207 no tiene respuesta
que esperar: no hay hipótesis con la que atacarla y su vía obvia está refutada, y por eso se mide sin
banda en los pasos 0 y 12. **Abierta hay una; cerrada con un no, otra; y no son la misma cosa.**
...
| **S-432** el dado de forma del día                    | `CUBIERTO` | Es una tirada, no una pieza; y es parcela de `entrenamiento.md`.                                                                                                                                                                                                                                                                                      |
...
| **S-467** la fuga en el puerto final                  | `PARCIAL`  | **CERRADA POR DECISIÓN, y con un no** (10-09-2026): **[DECISIÓN DEL DUEÑO 15] aceptada — la ley en cuesta NO se toca**. Se ataca por sus causas (R03.4d, R18, R28), **no con la perilla revertida**, y se mide en el paso 21. **Sale del «Cierra» de R13**, donde estaba a la vez que aquí. **Ya no cuenta como abierta**: la única abierta es S-207. |
...
`entrenamiento.md` §6 punto 5 dejó dictada para `SPRINTER_MIN` —«lo que el 68 quería decir es
...
- El objetivo de temporada repartido entre los suyos (S-028) ordena entrenamiento, convocatoria y
  pico de forma, y engancha con `entrenamiento.md` §5 sin contradecirlo.
...
Está medido, y la medida no se ha discutido desde entonces: **el banco no reproduce
...
`diseno/mapa-bancos.md` §7.1 lo confirma desde el otro lado, y con nombres: «el único campo pequeño de CI es
...
smallRaceSetup(raceId) → 5-10 equipos de 4-6 (según el raceId real), generados con
  generateNpcRider (Frontera 2 de `entrenamiento.md`), con autoStageOrders CON gcRank,
...
3. **`ordersBench.ts`** — el banco de órdenes que hoy **no existe** (`diseno/mapa-bancos.md` §7.7: «ningún
   ...
   **Y `orderDirection` con tolerancia cero solo es honesto si se mide pareado.** Meter en el job
   ...
   receta exacta del CI intermitente que este repositorio ya sufrió: `diseno/mapa-bancos.md` §7.21 avisa de
...
4. **`media-190` entra en CI** — hoy es informativo y sin banda (`diseno/mapa-bancos.md` §7.10), y es justo
   el escenario donde vive la deuda §14.4 (el ganador en solitario en media montaña, 4 % contra
   20-30 %). Con R18 esa deuda se ataca de frente, así que el escenario **tiene que vigilar**. **Y ya
   vigila**: [DUEÑO 20] aceptada el 10-09-2026 le da a `soloWinMediaPct` la banda de trabajo
   **12-28 %**, así que `media-190` entra **vigilando**, no solo midiendo. El defecto que este
   documento traía —«entra en CI midiendo, no vigilando», mientras la banda no estuviera firmada— no
   llegó a aplicarse.
...
| R17    | `medianLeadGroupRiders`                                   | `smallTours` ShapeStats + `realQueens` **partida por `queenFinalKind`** | **5-15 en reinas de final NO alto**; **sin banda en final en alto** (dec. 19 **aceptada el 10-09-2026**)                                               | **DEUDA (mide 1)** |
| R17    | `gregarioSharePct`                                        | `smallTours`                                                            | **sin banda** (§14 pto. 14 del mapa; su 45-65 % es la decisión 21, **aceptada el 10-09-2026: sigue sin banda, y ahora por decisión y no por defecto**) | no existe          |
| R18    | `soloWinMediaPct`                                         | **media-190** (entra en CI **vigilando**)                               | 12-28 % (dec. 20 **aceptada**; rebaja declarada de un objetivo del dueño)                                                                              | sin banda          |
...
(`diseno/mapa-bancos.md` §4.4 y §8)—, mientras que **todos** los disparadores de `trust` que R25.2 define
...
| **C2** | **La ley de velocidad no se ha movido en NINGUNA tanda táctica MENOS EL PASO 20**: invariante 43 (llana ≤ 48 km/h > media > reina ≥ 32) intacto en los otros veinte pasos | **Así hay que escribirlo desde que [DUEÑO 7] está aceptada (10-09-2026) y el paso 20 se hace.** El 20 es el **único** que mueve la LEY —la frontera que de verdad no se cruza, §1.1 Frontera 2— y **por eso va solo, el último y con `ENGINE_VERSION++` propio**: ahí el 43 se re-ancla con causa escrita y predicción declarada antes. **Si C2 se quedara escrito como «el 43 intacto» a secas, el paso 20 lo rompería y nadie sabría si lo que falla es el cambio o el invariante**, que es justo lo que un invariante de control existe para evitar. Y hay una segunda excepción, **de otro rango y no de la ley**: el **18d** (altitud) **edita `physics.ts`** y por eso es excepción declarada al «esto no ha filtrado a la física», pero tiene que dejar el **43 en banda**, declarado antes y comprobado después —toca el **coste** del bloque, no la ley—. La versión anterior de esta casilla decía «lo pueden tocar los pasos 18 y 20, y solo esos dos», y metía las dos excepciones en el mismo saco: **mover la ley** y **editar el fichero** no son lo mismo, y confundirlas hacía imposible saber qué se está comprobando en el 18d. Las dos constan en §9.2. |
...
en el nocturno, sumados desde `invariants.test.ts:244-256` y `diseno/mapa-bancos.md` §6:
...
(`railway.tick.json`), y `diseno/mapa-bancos.md` §4.2 avisa de que «el motor SE HA VUELTO MÁS LENTO… un 48 %
...
#### La factura de semillas que la decisión 22 NO trae, y por qué se escribe aquí

Esta subsección tenía apuntada una contrapartida condicional y hay que resolverla, porque §9.0 y §9.2
apuntan aquí para saber si se paga: **si el suelo de `mountain.top10GapSeconds` subía de 40 a 60**,
había que **subir las semillas del invariante 5** hasta que su mediana dejase de bailar —las 120
corridas dan una nube de **41 a 87 s** y el hueco medido en `… 52 52 55 │ 65 65 66 …` mueve la mediana
diez segundos cuando **una sola semilla** lo cruza—, y ese coste caía entero sobre este presupuesto.
**[DECISIÓN DEL DUEÑO 22], aceptada el 10-09-2026: el suelo se queda en 40 y solo sube el techo,
300 → 360.** Un techo que sube **no** necesita más semillas: aleja el listón de la nube en vez de
meterlo dentro. Así que **esta factura no se emite**: el invariante 5 sigue corriendo con las semillas
de hoy, su banda pasa a **40-360**, y ni el job diurno ni el nocturno pagan un segundo más por él. Se
deja escrito con el número delante porque la pregunta —«¿cuánto cuesta esa decisión en minutos de
CI?»— es exactamente la que hay que poder contestar sin volver a §10.
...
| **2**  | **Los contextos viajan, OPCIONALES, y nadie los lee.** `SelfView`/`GroupView`/`RaceView`; `race?` y `memory?` en `StageInput` **y en `StageInput` SOLO —`StageOutput` no gana ni un campo (§1.2, §9.6, Frontera 3): esta casilla decía «`StageInput`/`StageOutput`» y era el último resto de la contradicción con R09 que §1.2 ya corrigió**; `census(g)`; `teamId` en `MoveRider`; subflujos nuevos declarados y vacíos. **Cero cambios de conducta.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | ninguno (lo habilita todo)                                   | `stage/tactics.ts`, `finish.ts`, `simulate.ts`, `types.ts`                                                                                                                               | **nada**                                                                                                                                                                                                                                                             | Las cuatro huellas **idénticas dígito a dígito**; los 46 invariantes verdes; **C3** y **C4** en verde; `censusCost` ≤ 3 %; prueba de permutación (inv. 69) en verde; un test que afirma que `GroupView.mates` está poblado                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | **Sonnet**                                                                |
...
| **20** | **R14 — meteorología con previsión.** _([DECISIÓN DEL DUEÑO 7], **aceptada el 10-09-2026: SE HACE**. Deja de ser opcional. Va el **ÚLTIMO y SOLO**, con `ENGINE_VERSION++` propio, porque es el único paso que mueve la LEY de velocidad.)_ Viento con dirección; el abanico se cierra; lluvia que llega; material; parte citable.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **R14**                                                      | `weather.ts`, `physics.ts`, `simulate.ts`                                                                                                                                                | **huellas 4/4** · **la ley de velocidad (inv. 43)** y **todas** las bandas                                                                                                                                                                                           | `echelonClosedPct` ∈ 30-70 %; `windDayGapS` ∈ 1,5-4×; inv. 43 **re-anclado con causa escrita y predicción declarada ANTES**; **C2 se declara movido aquí y solo aquí**: C2 está redactado como «vigila todas las tandas tácticas menos el paso 20» (§7.6, §9.2), así que este paso no lo rompe —lo consume—, y lo que hay que enseñar es la predicción cumplida, no un invariante en rojo sin explicación                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | **Opus**                                                                  |
| **21** | **Calibrar y sellar.** Barrido de las constantes [calibrar]; las bandas de §9 escritas en `targets.ts`; `mountain.breakawayWinPct` re-anclada; **la SUSTITUCIÓN de `reina-150` por una reina de verdad** (3.500 m, dos puertos, final en alto), con la **huella de `reina-150` RETIRADA y la nueva SELLADA** (decisión 5, aceptada el 10-09-2026), y sobre ese escenario se decide qué banda lleva `mountain.breakawayWinPct`, que hasta aquí venía **sin banda** (decisión 23); nota v60 con tablas antes/después por paso; SPEC §6 reescrito a lo implementado; se retiran los comentarios desfasados que `diseno/mapa-bancos.md` §8 enumera.                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | —                                                            | `sim/targets.ts`, `docs/balance.md`, `SPEC.md`, `constants.ts`                                                                                                                           | todas, de forma declarada                                                                                                                                                                                                                                            | `pnpm sim 500` sin ningún objetivo fuera de banda; las **52** estadísticas de §7.5 **con banda escrita** (o publicadas sin banda, con su motivo, las que §10 deja **sin banda por decisión aceptada**: `gregarioSharePct` —decisión 21—, `rejoinAfterBigLossPct` —deuda §14.1— y `medianLeadGroupRiders` en reinas de final en alto —decisión 19—); ninguna constante sigue marcada [calibrar] sin número medido o sin decisión del dueño anotada; **el Apéndice B re-sumado contra `constants.ts` de verdad**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | **Sonnet**                                                                |
...
- **0 antes que todo**, porque es la lección de §7.1: el banco no reproduce lo que el
...
- **20 el último y SOLO** —**ya no «opcional»**: la decisión 7 está **aceptada el 10-09-2026**, el
  paso **se hace** y lleva `ENGINE_VERSION++` propio—, porque es el único que toca **la ley de
...
| **Tras el paso 19** | Todo menos el clima con dirección (R14)                                             | 20 de 20. **Ojo: el paso 20 ya no es opcional** (decisión 7, aceptada el 10-09-2026), así que este corte deja fuera **una tanda decidida**, no una tanda por decidir. |
...
### 9.0 La respuesta del dueño del 10-09-2026, y qué echa a andar

**Las veintitrés decisiones de §10 están contestadas: «acepto todas las recomendaciones».** Trece de
clase A —que ya corrían por defecto y siguen corriendo, con la diferencia de que ahora corren con un
sí delante— y **diez de clase B, que estaban paradas y ahora no**. Ésta es la lista de lo que esa
respuesta mueve, **con el sitio donde cada consecuencia está escrita**, porque una banda que se mueve
y solo se anota en §10 es exactamente el defecto que este documento existe para no cometer.

| Dec.   | Qué echa a andar                                                                                                                                                                                                                                                                                                      | Dónde está escrito                                                     |
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| **3**  | `breakawaySkipSprThreshold` deja de ser **70 absoluto** y pasa a **percentil p90 del campo del día**. El paso 6 **mezcla con la pieza**, no sin ella, y el veto al velocista puro vuelve a disparar en PRS y en CON                                                                                                   | §9.5 (constantes con historia) · §4/R03.4(a) · §8 paso 6               |
| **5**  | Se **sustituye `reina-150`** por una reina de verdad (3.500 m, dos puertos, final en alto) en el **paso 21**: huella vieja **retirada**, nueva **sellada**. La **disposición transitoria sigue en pie** y es el puente: `mountain.breakawayWinPct` medida en `realQueens` con el 12-35 % como lectura desde el paso 6 | §9.3 · §9.4 · §8 paso 21 · §10 dec. 5                                  |
| **6**  | Techo de la cola de las reinas **14 → 16 %**: `grandTour.queenLastGroupPct` 8-16 y `realQueens.lastGroupPct` 7-16. **El paso 10 ya no se para ahí**: criba todos los `shed` y **la banda nueva es la que vigila**                                                                                                     | §9.1 (2 filas) · §9.2 (inv. 25-30) · §8 paso 10 · §4/R26.5             |
| **7**  | **El paso 20 SE HACE**, el último y solo, con `ENGINE_VERSION++` propio. Y **C2 se reescribe**: pasa a vigilar **todas las tandas tácticas menos el paso 20**, que es el único que cruza la frontera de la ley                                                                                                        | §7.6 (fila C2) · §9.2 (fila 43) · §9.1 (ley de velocidad) · §8 paso 20 |
| **8**  | El corte de tiempo mira el número: `readmitBlockRiders` **20**. R26 se implementa **CON** la pieza, y **S-310, S-359, S-365, S-371 y S-412 pasan a cerrarse de verdad**                                                                                                                                               | §4/R26.1 y «Cierra» de R26 · §8 paso 10 · Apéndice B (fila R26)        |
| **19** | `medianLeadGroupRiders` **gana banda 5-15**, y **solo** en reinas cuyo final no sea en alto. En final en alto **sigue sin banda**, y eso también es la decisión                                                                                                                                                       | §9.1 · §9.1bis · §7.5 · §8 pasos 0 y 21                                |
| **20** | `soloWinMediaPct` **12-28 %** como banda de trabajo, **declarada como rebaja de un objetivo del dueño (20-30 %)** y no como número técnico. **`media-190` entra en CI vigilando**, ya no solo midiendo                                                                                                                | §9.1bis · §9.4 · §7.3 pto. 4 · §7.5 · §8 paso 21                       |
| **21** | `gregarioSharePct` **sin banda**, y **no** es criterio de «hecho» de ningún paso. Aceptar coincide aquí con no mover nada, **pero ahora es una decisión y no un defecto**                                                                                                                                             | §7.5 · §4/R17.1 · §8 paso 8 · Apéndice B pto. 1                        |
| **22** | Suelo de `mountain.top10GapSeconds` **se queda en 40**; **techo 300 → 360**. La banda del invariante 5 pasa a **40-360**, y **no hay que pagar más semillas** porque el suelo no sube                                                                                                                                 | §9.1 · §9.2 (inv. 1-7) · §7.7                                          |
| **23** | `mountain.breakawayWinPct` **re-anclada a `realQueens`** en el paso 6 y **publicada SIN banda**: no se escribe suelo ninguno. La banda vieja sobre `reina-150` queda **declarada INVÁLIDA**, no aflojada                                                                                                              | §9.1 · §9.4 · §8 pasos 6 y 21                                          |

**Dos de las trece de clase A merecen nota aparte, porque su respuesta aceptada no es un número sino
una consecuencia sobre lo que este documento declara abierto:**

- **La 15 (la ley en cuesta NO se toca)** cierra **S-467**: deja de estar abierta y pasa a estar
  **cerrada por decisión, y con un no**. Se ataca solo por sus causas (R03.4d, R18, R28) y se mide en
  el paso 21. **Lo único que sigue abierto en esta tanda es S-207**, y eso no cambia.
- **La 17 (partir el job de CI)** queda **aplazada a propósito**: su respuesta aceptada es «se decide
  **con los números del paso 0 delante**», o sea que **la decisión está tomada y lo que falta es el
  dato**. Eso no es una decisión sin contestar y no cuenta como tal en ninguna lista de este
  documento.
...
| **`grandTour.queenLastGroupPct`**                   | 8-14                    | **8-16**                                                                                                                               | 10              | Cribar todos los `shed` (deuda §14.2) sube la cola. **Es la única banda de esta lista con ancla de dominio**, el corte del §VI.3: **[DECISIÓN DEL DUEÑO 6]**. **ACEPTADA el 10-09-2026: el techo 16 se escribe**, el paso 10 deja de pararse ahí y es esta banda la que vigila la criba. El 14,33 % con 37 grupos en meta que ya salió al probarlo queda como la predicción declarada antes de correrlo.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **`realQueens.lastGroupPct`**                       | 7-14                    | **7-16**                                                                                                                               | 10              | Mismo motivo y misma decisión, y **misma aceptación del 10-09-2026**: 7-16.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- **1 se re-ancla de escenario** (`mountain.breakawayWinPct`: `reina-150` → `realQueens`), **y su
  SUELO no se re-ancla aquí**: bajarlo de 25 a 12 es **[DECISIÓN DEL DUEÑO 23]**. **Contestada y
  aceptada el 10-09-2026: no se escribe suelo ninguno, se publica sin banda.** La fila **sigue
  contando en las 34** porque la entrada de `sim/targets.ts` sigue existiendo y sigue midiendo; lo
  que no se escribe hasta el paso 21 es el par `min`/`max`, y eso va dicho en su fila.
...
banda que gana banda** (`medianLeadGroupRiders`, **decisión 19 aceptada**; las otras tres bandas
nuevas que este diseño crea van en §9.1bis) y la **ley de velocidad**, que no es una banda de
`targets.ts` sino el invariante 43 y **se re-ancla en el paso 20, que se hace** —decisión 7 aceptada
el 10-09-2026—. La redacción anterior decía «se re-ancla solo si se hace el paso 20», y ese «si» ya
está resuelto.
...
movió en persona, contra la tabla §13 de `diseno/mapa-requisitos-duenio.md`, y de esas trece **siete** están
...
| --------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | -------------- |
| `grandTour.queenLastGroupPct` + `realQueens.lastGroupPct` (la cola de la reina del CLI) | techo 14 → 16 — **ACEPTADO 10-09-2026**                                          | **[DUEÑO 6]**  |
| `mountain.top10GapSeconds` (suelo 40)                                                   | **no se sube**: firmó que se queda en 40; el techo 300 → 360 sí, y se hace       | **[DUEÑO 22]** |
| `mountain.breakawayWinPct` (suelo 25, banda provisional de la v33)                      | **no se baja a 12: se publica SIN banda** al re-anclar a `realQueens`            | **[DUEÑO 23]** |
...
**Y ninguna se ha movido tampoco por silencio.** Las tres decisiones que salían de esta tabla —6, 22 y
23— están **contestadas el 10-09-2026 y aceptadas**, así que las tres se mueven **con su firma
delante**: el techo de la cola de la reina sube al 16 %, el suelo de `mountain.top10GapSeconds` se
queda en 40 con el techo en 360, y `mountain.breakawayWinPct` se re-ancla y se publica sin banda. Era
justo lo que la regla perseguía, y conviene dejar constancia de que funcionó: el índice de las diez
consecuencias está en §9.0.
...
| ------------------------------------ | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `soloWinMediaPct`                    | **12-28 %**                                                     | `media-190` (entra en CI)                                                                  | **Rebaja de un objetivo del dueño** (pidió 20-30 %; hoy mide 4 %). El suelo de 12 es lo que se espera alcanzar con R18, no lo que dice la carretera. Por eso es **[DECISIÓN DEL DUEÑO 20]** y no una propuesta técnica. Se recalibra en el paso 21. **ACEPTADA el 10-09-2026: el 12-28 % es banda de trabajo desde ya, y la declaración se queda escrita tal cual** —aceptar una rebaja no la asciende a número de carretera, la deja como listón provisional con su procedencia a la vista—. Consecuencia: **`media-190` entra en CI VIGILANDO**, ya no solo midiendo (§7.3 pto. 4, §9.4). |
| `medianLeadGroupRiders`              | **5-15 en reinas de final NO alto; SIN banda en final en alto** | `smallTours` ShapeStats + `realQueens` partida por `queenFinalKind` (portada en el paso 0) | Deuda sin banda de la v23. **Procedencia: ENCARGO, no cita del dueño** (`targets.ts:591-593`), y encargo cuyo objetivo **nació rojo y por eso no llevaba banda**. El 5-15 es el del encargo, sin la rebaja a 3-10 que este documento traía. La partición por tipo de final la impone **v26 §5**: en final en alto el 1·1·1 dentro de 30 s es la carretera (Tour 2024) y no un defecto. **[DECISIÓN DEL DUEÑO 19], ACEPTADA el 10-09-2026: la banda 5-15 se escribe donde el final no es alto y la ausencia de banda en final en alto también es la decisión.**                              |
...
(decisión 21, publicada **sin** banda) y `finishPaveKm` (decisión 18, §9.5). **Las dos aceptadas el
10-09-2026**, y la 21 con una consecuencia que hay que decir en voz alta aunque el número no se mueva:
`gregarioSharePct` sigue **sin banda** y sigue **fuera** de los criterios de «hecho» del paso 8 y del
paso 21 —igual que antes—, pero ahora **porque se decidió que no la lleve**, no porque nadie hubiera
contestado. El 45-65 % se conserva como recomendación para cuando la pregunta del dueño se conteste, y
no antes.
...
| -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| 1-7 (canónicos)                                                      | Siguen; bandas movidas según §9.1. **El 5 (`mountain.top10GapSeconds`) es el único que cambia de banda, y solo por el techo**: 300 → 360. Su suelo 40 es decisión del dueño (§10, decisión 22) y **no se toca sin firma**. **Firmó el 10-09-2026 y aceptó: el suelo SE QUEDA EN 40**, así que la banda del 5 pasa a ser **40-360** y **no hay que subir las semillas** del invariante —esa contrapartida solo se pagaba si el suelo subía, y no sube (§7.7)—                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | var.   |
...
| 25-30 (gran vuelta)                                                  | Siguen; movidas `grandTour.abandonPct` (12-20 → 12-22) y `queenLastGroupPct` (8-14 → 8-16, **[DUEÑO 6]**, **aceptada el 10-09-2026: el 16 se escribe** y el paso 10 deja de pararse ahí). **Y el 28 se nombra aparte porque es el simplex**: el «se van por las tres puertas» mide `abandonCauses`, o sea `crashPct` (30-67 → **30-70**), `illnessPct` (20-67 → **18-67**, por arrastre) y `outOfTimePct` (1-15, **quieta**). Las tres se miden y se declaran **en la misma corrida**, según la regla de lectura de §9.1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 10-13  |
...
| **43 (ley de velocidad)**                                            | **C2 vigila TODAS las tandas tácticas MENOS el paso 20.** El 20 **se hace** ([DUEÑO 7], aceptada el 10-09-2026) y es el único que mueve la LEY: ahí el 43 se **re-ancla** con causa escrita y predicción declarada, en un PR solo, el último y con `ENGINE_VERSION++` propio. En los otros veinte pasos el 43 sigue **intacto** y C2 es exactamente lo que lo comprueba —**incluido el 18d**, que edita `physics.ts` por la altitud y por eso es excepción declarada al «nadie toca la física», pero que tiene que dejar el 43 **en banda** y declararlo antes y después: toca el **coste**, no la **ley**—. Se dice así, y no «intacto salvo paso 20», porque **si C2 se quedara escrito como una afirmación sin excepción el paso 20 lo rompería y nadie sabría si lo que falla es el cambio o el invariante**.                                                                                                                                                                                                                                                                                                                                                                                     | 20     |
...
**Y hay un re-sellado más que no es de esta lista y que la decisión 5, aceptada el 10-09-2026, pone
en el calendario: el del PASO 21.** Ahí se **sustituye `reina-150`** por una reina de verdad (3.500 m,
dos puertos, final en alto): la huella de `reina-150` **se RETIRA** —no se re-sella, se retira, porque
el escenario deja de ser canónico y pasa a llamarse `media-150`— y la de la reina nueva **se sella por
primera vez**, con su predicción declarada antes de correrla. No entra en el recuento de re-sellados
de `attribution.test.ts` de abajo porque no es un re-sellado: es una baja y un alta.
...
`smallTours.*` y la carta del día; el 20 mueve **las cuatro huellas y el invariante 43 de golpe**, y
así lo dice §8.1 — y **ya no lleva «si se hace» delante**: la decisión 7 está aceptada desde el
10-09-2026 y el paso 20 entra en el plan como los demás, el último y solo). Son dieciséis porque hay dieciséis cambios de conducta que tocan las
...
| `reina-150` | 135 km de llano + puerto de 15 km al 8 %, **1.200 m** | **[DECISIÓN DEL DUEÑO 5], ACEPTADA el 10-09-2026: se hace, en el paso 21.** Se renombra a `media-150` y se crea `reina-real` sobre un recorrido de 3.500-4.500 m con dos puertos y final en alto; la **huella de `reina-150` se RETIRA** y la de la reina nueva **se sella** (§9.3). Y el puente **se conserva porque la sustitución es en el 21 y la medida hace falta desde el 6**: `mountain.breakawayWinPct` se mide en `realQueens` desde el paso 6 (§9.1), con el 12-35 % como lectura y **sin banda escrita** (decisión 23). |
| `media-190` | informativo, sin banda, **fuera de CI**               | **Entra en CI con banda y VIGILANDO** (`mediaGroups`, `soloWinMediaPct` **12-28 %**, decisión 20 aceptada el 10-09-2026): es donde vive la deuda §14.4 y donde R18 se mide.                                                                                                                                                                                                                                                                                                                                                         |
...
| --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `breakawaySkipSprThreshold`                                           | 70 (absoluto)                                                                                           | Pasa a **percentil del campo del día** (p90 de SPR), para que funcione en continental. **[DECISIÓN DEL DUEÑO 3], ACEPTADA el 10-09-2026**: el paso 6 mezcla **con** la pieza                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `SPRINTER_MIN`                                                        | 68                                                                                                      | Ya marcado **[DECISIÓN DEL DUEÑO 25]** en `entrenamiento.md` §6: pasa a percentil. **Se hereda, no se reabre.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
**CONTESTADAS LAS VEINTITRÉS.** El dueño respondió el **10-09-2026**: «acepto todas las
recomendaciones». Las trece de clase A y las diez de clase B. Cada bloque de esta sección **conserva
su texto entero** —el porqué, las citas y las correcciones que traía—, y lo que se le ha añadido es la
respuesta: en la tabla, la columna de cabecera ya no dice «Si no contesta» sino **«Decidido»**, con lo
que ahora se hace; en la prosa, cada recomendación lleva detrás **DECIDIDO el 10-09-2026: aceptada**.

**Y aceptar no es sellar: mueve cosas.** Las diez de clase B estaban paradas y ahora echan a andar, y
cada consecuencia está escrita **donde vive** —§9.1, §9.1bis, §9.2, §8, §7 y el Apéndice B—, no aquí.
El índice de esas diez, con su sitio, es **§9.0**. Una banda que se mueve y solo se anota en §10 es
exactamente el defecto que este documento existe para no cometer.

**La partición en clase A y clase B ya no gobierna nada, y no se borra.** Gobernó mientras no había
respuesta y ésa era toda su función: que las bandas con ancla del dueño **no se movieran por
silencio**. Se queda escrita porque es **el registro de que no se movieron así** —se movieron con su
sí delante— y porque el criterio que la levantó vuelve a hacer falta la próxima vez que este
documento proponga tocar una banda suya. Esto es lo que decía, y va en pasado:

Cada una con recomendación, y **el defecto no era uno solo**. La versión anterior de esta sección
...
cola de la reina (decisión 6) era la única banda de §9.1 que el propio documento reconocía con ancla
de dominio, y su defecto era «sí, subir el techo a 16 %» — o sea, se subía sola si él no contestaba.
Eso contradecía de frente lo que el dueño dejó dicho en la v20 —**«no persigas el 45 % a ciegas…
Prefiero una especificación corregida a un motor calibrado hacia un objetivo equivocado»**— y lo que
`diseno/mapa-requisitos-duenio.md` §14.2 dice de esa banda concreta: está «atada a la banda 14 % de §VI.3 …
y **no se mueve sin decisión del dueño**».

**Así que el defecto se partió en dos clases, y cada decisión decía a cuál pertenecía:**

- **Clase A — defecto activo.** La recomendación se implementaba si no decía otra cosa. Eran las
  decisiones de **arquitectura, alcance y método**: no tocaban ninguna banda ni umbral que el dueño
  fijara o moviera en persona, y pararlas a la espera de una respuesta habría bloqueado el plan por
  nada.
- **Clase B — defecto PARADO.** La recomendación **no** se implementaba sin respuesta escrita, y **el
  paso que la contenía se paraba en ese punto** (el resto del paso podía mezclarse; lo que la decisión
  tocaba, no). Eran las que mueven una banda, un umbral o un corte que **él** fijó o movió: **6, 22,
  23, 5, 3, 8, 7, 19, 20 y 21**. La regla que las agrupa está en §1.4: «si la banda tenía ancla de
  dominio, lo que se discute es la regla», y una regla no se discute sola.

La columna decía **«Si no contesta»** y daba el valor literal con el que se seguía adelante, para que
«parado» significase algo concreto y no «alguien se acordará». **Contestó, así que ninguno de esos
diez valores llegó a aplicarse**; se conservan en los bloques de prosa que los traían (6, 17, 22 y 23)
porque son el contrafactual, y un contrafactual escrito es lo que permite saber, dentro de seis meses,
qué habría pasado de no haber firma.
...
| #                                        | Clase | Decidido — el dueño contestó el 10-09-2026: «acepto todas las recomendaciones»                                                                                                                                                                                                                      |
| ---------------------------------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 memoria de carrera                     | A     | **Aceptada.** Memoria de CARRERA: nace y muere con la carrera                                                                                                                                                                                                                                       |
| 2 cupo de fuga                           | A     | **Aceptada.** Cupo BLANDO (precio, no veto), salvo el del maillot, que sigue siendo veto por cita expresa                                                                                                                                                                                           |
| 3 `breakawaySkipSprThreshold` percentil  | **B** | **Aceptada.** `breakawaySkipSprThreshold` pasa de 70 absoluto a **percentil p90 del campo del día**; el paso 6 lleva la pieza y en continental el veto vuelve a disparar                                                                                                                            |
| 4 directores bot falibles                | A     | **Aceptada.** `dirQuality` {0,85 · 0,65 · 0,50}, con el brazo de control y el invariante 47 como mitigación obligatoria                                                                                                                                                                             |
| 5 sustituir `reina-150`                  | **B** | **Aceptada.** Se **sustituye `reina-150`** por una reina de verdad (3.500 m, dos puertos, final en alto) en el **paso 21**: huella vieja **retirada**, nueva **sellada**. La disposición transitoria sigue en pie y es el puente: `mountain.breakawayWinPct` medida en `realQueens` desde el paso 6 |
| 6 cola de la reina 14 → 16 %             | **B** | **Aceptada.** Techo de la cola de la reina **14 → 16 %** (`grandTour.queenLastGroupPct` 8-16 y `realQueens.lastGroupPct` 7-16). El paso 10 **ya no se para ahí**: criba y la banda nueva es la que vigila                                                                                           |
| 7 ¿se hace el paso 20?                   | **B** | **Aceptada: EL PASO 20 SE HACE**, el último y solo, con `ENGINE_VERSION++` propio. **C2 pasa a vigilar todas las tandas tácticas menos ésa**                                                                                                                                                        |
| 8 corte de tiempo por número             | **B** | **Aceptada.** `readmitBlockRiders` **20**; R26 se implementa CON la pieza, y S-310, S-359, S-365, S-371 y S-412 se cierran de verdad en vez de «por fuera»                                                                                                                                          |
| 9 rechazar el liderazgo                  | A     | **Aceptada.** `pickLeader` ignora los votos que apuntan a quien se declaró `libre` o `cazaetapas`, con la contrapartida escrita en pantalla                                                                                                                                                         |
| 10 retirar `tacticMaxMoves`/`closingNow` | A     | **Aceptada.** Se retiran los dos, en el paso 5                                                                                                                                                                                                                                                      |
| 11 CRE                                   | A     | **Aceptada.** La CRE queda fuera de alcance, anotada con su ficha en el catálogo y su línea en la deuda                                                                                                                                                                                             |
| 12 comisario                             | A     | **Aceptada.** El comisario, no todavía                                                                                                                                                                                                                                                              |
| 13 re-sellado por tanda                  | A     | **Aceptada.** Re-sellado por tanda, con causa escrita Y predicción declarada antes                                                                                                                                                                                                                  |
| 14 `orderEffectSpread` invariante        | A     | **Aceptada.** Invariante, con las tres caras, sobre las **once** palancas                                                                                                                                                                                                                           |
| 15 la ley en cuesta                      | A     | **Aceptada: la ley en cuesta NO se toca.** Y con eso **S-467 queda CERRADA por decisión, y con un no**: se ataca solo por sus causas y se mide en el paso 21                                                                                                                                        |
| 16 general a `realQueens`                | A     | **Aceptada.** General sintética a `realQueens`; no a `calendarQueens` ni a `climbs`                                                                                                                                                                                                                 |
| 17 partir el job de CI                   | A     | **Aceptada, y por eso APLAZADA A PROPÓSITO**: se decide **con los números del paso 0 delante**. La decisión está tomada; lo que falta es el dato, no la respuesta                                                                                                                                   |
| 18 `finishPaveKm` 30 → 10                | A     | **Aceptada.** `finishPaveKm` 30 → 10, con la comprobación de los cinco recorridos; si Roubaix cae, vuelve a 30                                                                                                                                                                                      |
| 19 `medianLeadGroupRiders` gana banda    | **B** | **Aceptada.** `medianLeadGroupRiders` **gana banda 5-15**, y **solo** en reinas cuyo final no sea en alto; en final en alto sigue **sin banda**                                                                                                                                                     |
| 20 `soloWinMediaPct` suelo 12            | **B** | **Aceptada.** `soloWinMediaPct` **12-28 %** como banda de trabajo, y la declaración se queda: es una **rebaja de un objetivo del dueño (20-30 %)**. `media-190` entra en CI **vigilando**                                                                                                           |
| 21 `gregarioSharePct` gana banda         | **B** | **Aceptada.** `gregarioSharePct` **sin banda**, y no es criterio de «hecho» de ningún paso. Aquí aceptar coincide con no mover nada, pero ahora es una **decisión** y no un defecto                                                                                                                 |
| 22 suelo de `mountain.top10GapSeconds`   | **B** | **Aceptada.** El suelo de `mountain.top10GapSeconds` **se queda en 40**; el techo sube **300 → 360**                                                                                                                                                                                                |
| 23 suelo de `mountain.breakawayWinPct`   | **B** | **Aceptada.** `mountain.breakawayWinPct` **re-anclada a `realQueens`** en el paso 6 y **publicada SIN banda**; la banda vieja sobre `reina-150` queda declarada **INVÁLIDA**, no aflojada                                                                                                           |

**Y una regla de lectura para las de clase B**, que rigió mientras no hubo respuesta y que no se
borra porque explica por qué el plan nunca llegó a estar bloqueado: _parado_ no era _aparcado_. El
paso corría igual, **medía** y publicaba el número **sin banda**, con su predicción escrita. Lo único
que no ocurría sin firma era **escribir la banda nueva en `sim/targets.ts`**. Medir nunca necesita
permiso; cambiar el listón, sí. **Con la firma del 10-09-2026 las diez pasan de medir a vigilar**, y
cada una lleva su banda escrita en el paso que le toca: la lista, con el sitio, está en §9.0.
...
→ **Recomendación: memoria de CARRERA, no de temporada.** · **DECIDIDO el 10-09-2026: aceptada.** `RaceMemory` nace y muere con la carrera;
...
→ **Recomendación: blando.** · **DECIDIDO el 10-09-2026: aceptada.** El cupo es un **precio** (`appetite = 0` al superarlo) y no un veto,
...
→ **Recomendación: sí, percentil (p90 del campo del día).** · **DECIDIDO el 10-09-2026: aceptada.** Es la misma corrección que
`entrenamiento.md` §6 ya dictó para `SPRINTER_MIN`, y no reabrirla sería tener dos umbrales
absolutos incoherentes en el mismo motor.
→ **Y lo que eso echa a andar**: el **paso 6 mezcla CON la pieza**, no sin ella, así que el veto al
velocista puro vuelve a disparar en PRS y en CON. Se anota en §9.5 (la fila de la constante), en R03.4(a)
y en el criterio de «hecho» del paso 6, donde hay que publicar el porcentaje de sprinters en la fuga
por división **antes y después** del cambio de umbral. Deja de ser cierto lo que este bloque decía
como defecto —«en continental los sprinters siguen yéndose a la fuga, medido y anotado»—: ya no se
mide un defecto, se corrige.

**4. ¿Los directores bot fallan, y por qué?** (S-009, S-013, S-014, G9)
→ **Recomendación: sí, y por INFORMACIÓN, no por vatios.** · **DECIDIDO el 10-09-2026: aceptada.** `dirQuality` por división
...
→ **Recomendación: sí, sustituirlo por una reina de verdad (3.500 m, dos puertos, final en alto)** · **DECIDIDO el 10-09-2026: aceptada.**
...
→ **Esta disposición transitoria NO se borra al aceptar, y hay que decir por qué**: la sustitución
ocurre en el **paso 21** y la medida hace falta desde el **paso 6**, así que los quince pasos que hay
en medio necesitan un sitio donde leer el número. El 12-35 % es ese sitio: **la lectura de puente**,
la horquilla contra la que se interpreta lo que salga, no una banda escrita en `sim/targets.ts` —eso
lo gobierna la decisión 23, también aceptada, y dice **publicar sin banda**—. Las dos cosas conviven
sin contradecirse porque hablan de cosas distintas: **con qué se lee** el número (12-35 %, decisión 5)
y **qué se escribe** en el fichero de objetivos (nada, hasta el paso 21, decisión 23). En el paso 21,
con la reina de verdad ya sellada, es cuando se decide qué banda se escribe y sobre qué escenario.
→ **Contrafactual conservado**, que era el valor de la columna cuando aún no había respuesta:
«**`reina-150` se queda como escenario canónico y con su nombre**; `mountain.breakawayWinPct` se
publica **sin banda** en `realQueens` hasta que conteste». **No llegó a aplicarse.**
...
→ **Recomendación: sí, subir el techo a 16 %** · **DECIDIDO el 10-09-2026: aceptada.** (`grandTour.queenLastGroupPct` y
`realQueens.lastGroupPct`). El corte del §VI.3 va del 8 % (llana) al 18 % (reina): 16 % **sigue
dentro del corte** y es la carretera.
→ **Clase B. Contrafactual conservado**, que era el valor de la columna cuando aún no había
respuesta: «**el techo se queda en 14 %**». **No llegó a aplicarse.** Y hay que corregir dos cosas que
esta decisión decía: (a) **no es «la única banda de §9.1 con ancla de dominio»** —son cuatro filas y tres
decisiones, 6, 22 y 23, contadas en §9.1 contra la lista de trece de §1.4—; y (b) **su defecto no
puede ser «sí»**, porque `diseno/mapa-requisitos-duenio.md` §14.2 dice de esta banda exactamente que está
...
grupos en meta es lo que ya salió al probarlo— y **se paraba ahí** hasta que él contestase.
→ **Contestó, y con un sí, así que el paso 10 YA NO SE PARA AHÍ.** El techo pasa a **16 %** en las dos
filas (`grandTour.queenLastGroupPct` 8-16 y `realQueens.lastGroupPct` 7-16), la criba de **todos** los
`shed` de R26.5 se mezcla entera, y **la banda nueva es la que vigila**: si la cola se sale del 16 %,
lo que está mal es el cambio y no la banda, que es el trato de siempre. El 14,33 % medido se conserva
escrito como la predicción declarada antes de correrlo. El contrafactual de arriba —«el techo se queda
en 14 %»— **no llegó a aplicarse**, y se conserva por lo que era: el valor con el que se habría
seguido de no haber firma. Queda anotado en §9.1 (dos filas), §9.2 (invariantes 25-30) y en el
criterio de «hecho» del paso 10.
...
→ **Recomendación: sí, pero el ÚLTIMO y SOLO, con `ENGINE_VERSION++` propio.** · **DECIDIDO el 10-09-2026: aceptada.** R14 cierra 12
...
→ **Y aceptado tiene una consecuencia que hay que escribir con cuidado, porque toca un invariante de
control.** C2 dice hoy «la ley de velocidad no se ha movido: invariante 43 intacto». Con el paso 20
aceptado eso **deja de ser cierto para ese paso**, y un invariante que se rompe sin decirlo no vigila
nada. Así que C2 se redacta como lo que es: **C2 vigila TODAS las tandas tácticas MENOS el paso 20**.
El 20 es el único que cruza la frontera de la ley (§1.1, Frontera 2), y **por eso va solo, el último y
con `ENGINE_VERSION++` propio**: ahí el 43 se re-ancla con causa escrita y predicción declarada antes.
En los otros veinte pasos el 43 sigue intacto y C2 es lo que lo comprueba —incluido el **18d**, que
edita `physics.ts` por la altitud y por eso es excepción declarada al «nadie toca la física», pero que
tiene que dejar el 43 **en banda** y declararlo antes y después: toca el coste, no la ley—. Si C2 se
quedara escrito como está, **el paso 20 lo rompería y nadie sabría si lo que falla es el cambio o el
invariante**. Escrito en §7.6 (fila C2), §9.2 (fila del 43), §9.1 (fila de la ley de velocidad) y en
el criterio de «hecho» del paso 20.
...
→ **Recomendación: sí.** · **DECIDIDO el 10-09-2026: aceptada.** `readmitBlockRiders` 20: al grupo grande se le readmite en bloque con
...
→ **Y aceptado, R26 se implementa CON la pieza**: `readmitBlockRiders` **20** entra en el paso 10 y
**S-310, S-359, S-365, S-371 y S-412 pasan a cerrarse de verdad**, con la causa que les faltaba —el
grupeto tiene algo que perder y por eso se organiza por miedo y no por inercia—. El contrafactual
—«se queda como hoy, readmite siempre y a todos, y se dice en la crónica que el grupeto no tiene
apuesta»— **no se aplica**. Escrito en R26.1, en el «Cierra» de R26, en el criterio de «hecho» del
paso 10 y en el Apéndice B (la constante ya contaba en la fila R26).

**9. ¿Un humano puede rechazar el liderazgo?** (S-058)
Hoy, si sus gregarios bot le apuntan, es jefe aunque escriba `libre`.
→ **Recomendación: sí.** · **DECIDIDO el 10-09-2026: aceptada.** `pickLeader` ignora los votos que apuntan a quien se declaró `libre` o
...
restantes».
→ **Recomendación: sí, los dos, en el paso 5.** · **DECIDIDO el 10-09-2026: aceptada.** El techo pasa a ser por fase y por grupo de origen;
...
→ **Recomendación: no ahora.** · **DECIDIDO el 10-09-2026: aceptada.** Queda anotada como fuera de alcance, con su ficha en el catálogo y su
...
→ **Recomendación: no, todavía.** · **DECIDIDO el 10-09-2026: aceptada.** Es un actor nuevo en una capa que no existe, aporta poco a la
...
→ **Recomendación: por tanda, con causa escrita Y PREDICCIÓN DECLARADA ANTES** · **DECIDIDO el 10-09-2026: aceptada.** Como se hizo en la
...
→ **Recomendación: invariante, y con las TRES caras.** · **DECIDIDO el 10-09-2026: aceptada.** Suelo `orderEffectSpread` ≥ 0,25 (una palanca
...
→ **Recomendación: NO tocarla, y atacar S-467 solo por sus causas** · **DECIDIDO el 10-09-2026: aceptada.** —el hombre correcto en la fuga
...
→ **Y aceptado esto, S-467 deja de estar abierta: está CERRADA POR DECISIÓN, y con un no.** Es una
distinción que hay que sostener en todo el documento, porque «abierta» y «cerrada con un no» se leen
igual desde lejos y no son lo mismo: una espera respuesta, la otra la tiene. La única situación que
sigue **abierta** en esta tanda es **S-207** (el reagrupamiento tras perder cinco minutos), y eso no
cambia: su vía obvia está refutada, no hay hipótesis con la que atacarla y se mide sin banda en los
pasos 0 y 12 para no empeorarla a ciegas. Corregido en §4/R13, en la tabla de cobertura de §4, en la
tabla de «lo que queda fuera a propósito» y en el Apéndice A.
...
→ **Recomendación: sí, a `realQueens`; no a `calendarQueens` ni a `climbs`.** · **DECIDIDO el 10-09-2026: aceptada.** `realQueens` son 9
...
pruebas**, y el propio `diseno/mapa-bancos.md:500` lo marca como caducado. Hoy `test:bancos` corre cinco
...
→ **Recomendación: sí a la partición, pero DECIDIDA DESPUÉS DE MEDIR, no antes** (§7.7). · **DECIDIDO el 10-09-2026: aceptada.** El paso 0
...
→ **Clase A. Contrafactual conservado**, que era el valor de la columna cuando aún no había
respuesta: «**se decide con los números del paso 0 delante**». Aquí el contrafactual y lo decidido
dicen lo mismo, y por eso hay que leer la línea de abajo para saber cuál de los dos rige.
→ **DECIDIDO el 10-09-2026: aceptada — y por eso queda aplazada A PROPÓSITO.** Es el único caso de
esta sección en el que la respuesta aceptada **no es un número sino un procedimiento**, y conviene no
confundirlo: **la decisión está tomada** («sí a la partición, decidida después de medir») y lo que
falta es **el dato**, que llega en el paso 0. No es una decisión sin contestar y no cuenta como tal en
ninguna lista de este documento; el sitio donde el dato aterriza es §7.7, y la condición dura se
conserva entera: todo paso que mueva una banda del job largo **corre el largo antes de mezclar**.
...
→ **Recomendación: sí, 30 → 10. Y NO a 3** · **DECIDIDO el 10-09-2026: aceptada.**, que es lo que este documento proponía con un ejemplo
...
`alto`** · **DECIDIDO el 10-09-2026: aceptada.** (`queenFinalKind ∈ {cima_cerca, valle_corto, valle_largo}`, que R28.2 hace explícito y
...
→ **Aceptada, así que `medianLeadGroupRiders` GANA BANDA**: **5-15**, y **solo** en reinas cuyo final
no sea en alto (`queenFinalKind ∈ {cima_cerca, valle_corto, valle_largo}`). **En final en alto sigue
sin banda**, y eso también es la decisión, no un olvido: v26 §5 midió 1·1·1 dentro de 30 s con el Tour
2024 delante, y ponerle banda ahí sería llamar defecto a la carretera. Deja de ser deuda sin banda y
pasa a objetivo partido por tipo de final: escrito en §9.1, §9.1bis, §7.5 y en el paso 0 (la portada
a `realQueens` partida por `queenFinalKind`), y sellado en el paso 21.
→ **Contrafactual conservado**: «**se publica sin banda** (que es como está hoy) en los dos tipos de
final; no se convierte un encargo rojo en objetivo por silencio». **No llegó a aplicarse** — y la
frase importa, porque el encargo se convierte en objetivo **con firma**, que es justo lo contrario de
por silencio.
...
objetivo del dueño, no un número técnico.** · **DECIDIDO el 10-09-2026: aceptada.** Va en §9.1bis con esa procedencia y **se recalibra en el
...
→ **Aceptada: 12-28 % es banda de trabajo desde ya**, y **la declaración se queda escrita tal cual**
—**es una rebaja de un objetivo que pidió el dueño (20-30 %)**, no un número técnico—, en §9.1bis y en
§9. Aceptarla no la asciende a número de carretera: la convierte en un listón provisional con su
procedencia a la vista, que es distinto. Consecuencia operativa: **`media-190` entra en CI
VIGILANDO**, ya no solo midiendo (§7.3 punto 4, §9.4), y se recalibra en el paso 21 contra lo que de
verdad haya salido. Si con las cuatro piezas de R18 el número se queda cerca de 12, la conversación
que toca es la de la decisión 15 —la ley en cuesta, contestada con un no—, no la de ensanchar.
→ **Contrafactual conservado**: «**se publica sin banda**; `media-190` entra en CI **midiendo, no
vigilando**». **No llegó a aplicarse.**
...
ningún paso.** · **DECIDIDO el 10-09-2026: aceptada.** El 45-65 % que este documento traía como banda queda aquí como **recomendación para
...
→ **Aceptada, y aquí aceptar coincide con no mover nada — pero la diferencia importa y va dicha.**
`gregarioSharePct` sigue **sin banda** y sigue **fuera de los criterios de «hecho»** de los pasos 8 y
21, exactamente igual que antes; lo que cambia es **por qué**: antes era el defecto de una decisión
sin contestar, y ahora es **una decisión tomada**. El 45-65 % se conserva como recomendación para
cuando la pregunta del dueño se conteste, y no antes. Escrito en §7.5, en R17.1, en el criterio de
«hecho» del paso 8 y en el punto 1 del Apéndice B.
→ **Contrafactual conservado**: «**se publica sin banda** — que es además la recomendación, así que
aquí las dos coinciden». Coincidían en el número y no en su estatuto, y ésa es toda la diferencia que
esta decisión aporta: el mismo «sin banda», con firma detrás.
...
intermitente**.
→ **Recomendación: NO subirlo.** · **DECIDIDO el 10-09-2026: aceptada.** El **techo sí sube, 300 → 360**, y eso se queda en §9.1 como
...
estabilidad.
→ **Clase B. Contrafactual conservado**, que era el valor de la columna cuando aún no había
respuesta: «**el suelo se queda en 40**».
→ **DECIDIDO el 10-09-2026: aceptada.** El **suelo se queda en 40** —y aquí el contrafactual y lo
decidido coinciden, pero por motivos opuestos: uno era silencio y esto es firma— y el **techo sube
300 → 360**. La banda del invariante 5 pasa a ser **40-360**, escrita en §9.1 y en §9.2. Y la
contrapartida de arriba **deja de estar en el aire**: como el suelo no sube, **no hay que subir las
semillas del invariante 5** ni pagar esa estabilidad en §7.7. El motivo se conserva entero porque es
la parte que importa: un suelo de 60 vuelve a sentarse en el centro de la nube de 41-87 s y
reintroduce el fallo intermitente.
...
montaña**, y esa banda tiene ancla de dominio. `diseno/mapa-requisitos-duenio.md` §2, v33, la recoge:
...
→ **Recomendación: re-anclar el escenario ya (paso 6) y PUBLICAR SIN BANDA hasta que él contestase.** · **DECIDIDO el 10-09-2026: aceptada** —contestó, y sigue sin banda: no hasta la respuesta, sino **hasta el paso 21**, que es donde se decide qué banda se escribe y sobre qué escenario.
...
→ **Clase B. Contrafactual conservado**, que era el valor de la columna cuando aún no había
respuesta: «**se publica sin banda, y la banda vieja sobre `reina-150` queda declarada INVÁLIDA, no
aflojada**». Una banda que mide la posición del puerto no se conserva por inercia.
→ **DECIDIDO el 10-09-2026: aceptada.** El **re-anclaje a `realQueens` se hace en el paso 6** y el
número **se publica SIN banda**: no se escribe suelo ninguno en `sim/targets.ts`, ni el 25 viejo ni el
12 propuesto. La **banda vieja sobre `reina-150` queda declarada INVÁLIDA** —no aflojada, no
conservada por inercia—, porque medía la posición del puerto y no la carrera. El 12-35 % de la
disposición transitoria de la decisión 5 sigue siendo **la lectura de puente** con la que se
interpreta lo que salga entre el paso 6 y el paso 21; qué banda se escribe, y sobre qué escenario, se
decide en el **paso 21** con la reina de verdad ya sellada. Y si el número se queda abajo, la
conversación que toca es la de la decisión 15 —contestada, y con un no—, no la de aflojar el suelo.
Escrito en §9.1, §9.4 y en los criterios de «hecho» de los pasos 6 y 21.
...
**Y ninguna de las veinte queda pendiente de una decisión sin contestar.** Las veintitrés de §10
están respondidas el **10-09-2026**, todas aceptadas, así que cada fila de esta tabla muere en el paso
que dice y no en «cuando el dueño conteste». Dos consecuencias que hay que leer aquí porque cambian lo
que este apéndice da por abierto:

- **S-467 no está en esta tabla y tampoco está abierta**: la decisión 15 se aceptó —**la ley en cuesta
  no se toca**— y con eso queda **cerrada por decisión, y con un no**. Se ataca por sus causas y se
  mide en el paso 21.
- **Lo único abierto de todo el documento es S-207**, que tampoco está en esta tabla: no espera
  respuesta de nadie, espera una hipótesis, y mientras tanto se mide sin banda en los pasos 0 y 12.

La decisión **17** (partir el job de CI) es el único caso que se parece a un pendiente y no lo es:
está **aceptada y aplazada a propósito** —«se decide con los números del paso 0 delante»—, así que lo
que falta es el dato y no la respuesta.
...
| R14  | 7 · 1 D · 6 c   | 7 · **0 D** · **7 c**        | `echelonCloseThreshold` deja de derivarse de `POS.vientoMinimo`, que **tampoco existe**. R14 se queda **sin ninguna DERIVADA**, y eso es información: el racimo del viento no tiene un solo ancla en el motor de hoy. **Y desde el 10-09-2026 estas siete dejan de ser condicionales**: la decisión 7 está **aceptada**, el paso 20 se hace, y las siete cuentan en firme en el total —antes colgaban de un «si se hace R14».                                                                                                                                                                                            |
...
**Y el re-sumado del paso 21 se hace ya sobre los veintiún pasos, no sobre veinte y un condicional.**
Con las veintitrés decisiones contestadas el **10-09-2026** —todas aceptadas— ninguna constante de
este recuento cuelga de una respuesta que falte: el paso 20 se hace (decisión 7), así que las **7 de
R14** cuentan en firme; `readmitBlockRiders` **20** entra con el paso 10 (decisión 8) y ya estaba
contada en la fila **R26**; y `breakawaySkipSprThreshold` (decisión 3) no suma como nueva porque es
una constante de hoy que cambia de naturaleza, y por eso vive en §9.5 y no aquí. El criterio de
«hecho» del paso 21 que decía «ninguna constante sigue marcada [calibrar] sin número medido **o sin
decisión del dueño anotada**» conserva las dos mitades, pero la segunda ya está satisfecha entera: las
decisiones están anotadas, con fecha, y lo que cada una mueve está en §9.0.
...
hoy**, y el propio `diseno/mapa-bancos.md` §7.21 avisa de que «todo lo más fino que 2-4 puntos porcentuales
...
1. **El paso 21 no las barre todas.** Barre las que tienen banda propia en §7.5 —**48 de las 52
   ...
   que la vigile no se calibra: se deja quieta y se anota. **Y el reparto no cambia con la respuesta
   del dueño del 10-09-2026, pero sí cambia por qué es así**: `gregarioSharePct` sigue sin banda
   **por decisión aceptada** (la 21) y no por una decisión sin contestar; `medianLeadGroupRiders`
   sigue mixta **por decisión aceptada** (la 19), con el 5-15 escrito donde el final no es alto y sin
   banda donde sí lo es. Sigue siendo **48 de 52**, y ahora las cuatro excepciones tienen firma.
...
| **10.** «Invertir el defecto de §10 **solo** para las decisiones 6, 8, 5, 3 y 7; el resto pueden mantener defecto activo.»                                                                                                                                                                     | Revisor **contra el dueño** (tactica.md:4166 y mapa §14.2)                                                          | Se aplica, **y se amplía**, que es la única parte que se desestima: la lista de clase B no son cinco decisiones sino **diez** —6, 22, 23, 5, 3, 8, 7, 19, 20 y 21—. El criterio del revisor es correcto («las que tocan una banda o un umbral que el dueño fijó o movió en persona») pero su lista se quedó corta porque se escribió antes de que §1.4 pasara de tres anclas de dominio a **trece**, y antes de que existieran las decisiones 19, 20, 21 (encargo rojo, rebaja de un objetivo suyo, pregunta que él dejó abierta) y las 22 y 23 (los dos suelos de montaña). Aplicar el criterio a rajatabla da diez, no cinco. Y se añade la columna «Si no contesta» con el valor literal, que es lo que el revisor pedía. **Epílogo, y es lo que valida la objeción entera**: el dueño contestó el **10-09-2026** y aceptó las veintitrés, las diez de clase B incluidas. La partición dejó de gobernar ese mismo día, pero hizo su trabajo —**ninguna banda con ancla suya se movió por silencio**— y por eso se conserva escrita en pasado. Lo que las diez echan a andar está en **§9.0**.                                                                                                                                                                                                       |
...
**Y la comprobación de que esta lista está completa, hecha al final y contra el texto, no contra los
informes.** Los 86 fallos se han vuelto a auditar uno a uno sobre el documento de hoy —no sobre los
números de línea que traían, que se han corrido: el documento ha pasado de 5.512 a más de 6.300
líneas— buscando, para cada uno, o su corrección escrita en el sitio que le toca o su fila aquí.
Resultado: **71 aplicados enteros, 15 aplicados en parte —las catorce filas de arriba, con la 8
cubriendo dos fallos (el 28 y el 58 son el mismo defecto de R04.5 visto por dos refutadores)— y
ninguno sin tocar**. Los **cinco** que la cabecera daba por no aplicados porque no aparecían en
ningún informe de corrección **están cerrados**, y el motivo de que nadie los reportara es que son
**segundas lecturas de un defecto que otro refutador ya había sacado**: cuatro refutadores mirando el
mismo documento repiten hallazgos, y el arreglo se escribe una sola vez. Los seis defectos con más de
un enunciado son el **coste del bloque contra la Frontera 2** (§9.1bis), la **línea base de CI**
(§7.7 y el paso 0), la **numeración de invariantes** (§7.6 como fuente única), **R04.5**, el **suelo
de `mountain.top10GapSeconds`** y la banda de **`medianLeadGroupRiders`**. La única cosa que la
auditoría encontró a medio propagar, y cerró, fue el resto de la contradicción de `StageOutput` en la
casilla del **paso 2** de §8: §1.2 y §9.6 ya decían que **`StageOutput` no gana ni un campo** y esa
casilla seguía diciendo lo contrario. **Un recuento que no se vuelve a contar contra el texto es una
promesa, no un recuento**, y por eso esta comprobación va escrita aquí en vez de darse por hecha.
