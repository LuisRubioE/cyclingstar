...
// EditionPlan de './edition.js', StageRequest de './generate.js'. El único valor nuevo que motifs.ts importa es degradarMotivo de './geo.js'.
```
...
import type { GeoSignature, Requiere } from './geo.js'
...
export interface Skeleton {
  ...
  requiere?: Requiere | Requiere[]             // lista = alternativas («o bien»); tipo en geo.ts (§3.4)
...
- `requiere` es un `Requiere` (o una lista de ellos, que son alternativas: basta con que una se cumpla), tipo propio de `geo.ts` (§3.4) y no `Partial<GeoSignature>`, porque este no puede decir «no nulo», un umbral ni una disyunción. Lo lee solo `admite(geo, sk)`: `{ adoquin: 2 }` exige `geo.adoquin ≥ 2`, `{ sterrato: true }` exige `sterrato`, `{ finalesAlto: 'largo' }` exige exactamente ese valor, `{ cota: true, cotaKmMin: 3.3 }` exige una cota de la zona que llegue a 3,3 km, y además todo `Slot` de `puerto`, `cota` o `muro` con `n[0] ≥ 1` hace indisponible el esqueleto donde ese campo de la zona es `null`.
...
/** Lo que un esqueleto exige de la zona (sección 6, §6.5 punto 1); un campo ausente no exige nada. */
export interface Requiere {
  puerto?: true                                // geo.puerto !== null
  cota?: true                                  // geo.cota !== null
  cotaKmMin?: number                           // geo.cota !== null && geo.cota.km[1] >= n
  ...
  relieveMin?: Relieve                         // orden(geo.relieve) >= orden(r): llano < ondulado < media < montana < alta
...
/** (1) sk.requiere (o alguna de sus alternativas) se cumple campo a campo; (2) todo Slot con n[0] >= 1 de puerto, cota o muro
 *  tiene geo[motif] !== null y rango efectivo de km no vacío; (3) la meta cabe en su motivo de la zona (sección 6, §6.5 punto 3). */
...
`admite` recibe `(geo, sk)` en ese orden en todo el documento: la sección 5 escribía `admite(requiere, geo)` y la pasada de coherencia la alinea. `Requiere` lleva los campos de la sección 6 y no los de la sección 5 (que escribía `muro?: true | { adoquin: true }`, `relieve?` y `altitud?` de un solo valor): las cuatro enmiendas de §6.5 (la lista de `et_media_tendida`, `relieveMin: 'alta'` en `et_reina_encadenada`, `cotaKmMin: 3.3`, `muro: 'adoquin'`) solo se pueden escribir con estos. Hay tres funciones de degradación con tres nombres: `degradar` (relieve, aquí), `degradarMotivo` (hueco, aquí) y `degradarPapel` (papel, `skeletons.ts`, §3.3).
...
/** n = etapas de la vuelta (1 en un día y en los nacionales), lo que mixKm(role, n, last, rand) recibe hoy (calendar.ts l. 536-543). */
...
Los subflujos son una lista cerrada porque el principio 7 (puro y determinista con subflujos nominales) solo se cumple si añadir una tirada en un sitio no desplaza las demás: es el defecto de `profileGen.ts` l. 316-317 que el diagnóstico mide. Los subflujos sin `season` (`arch`, `firma`) son la identidad; los cuatro con `season` (`ed`, `mot`, `pos`, `dib`) son la edición; el sufijo `i{intento}` solo existe en `mot`, `pos` y `dib` (sección 8). La clave de etapa separa la composición de la vuelta (`arch|${raceId}`, que consume `itinerarioDe` de una vez) de la elección de esqueleto de cada etapa (`arch|${id}`): sin ella, una tirada más en la composición movería el esqueleto de todas las etapas. `ed` es una corriente POR ETAPA (`ed|${id}|${season}`) y no una por carrera consumida en orden de etapa, que es lo que este bloque decía antes: con una por carrera, añadir un hueco al esqueleto de la etapa 1 movería las cardinalidades de todas las siguientes (sección 10, §10.4). El km base de un día va en `firma|${raceId}|km` y no en `firma|${raceId}` porque `routeRng` da la misma secuencia a la misma cadena, y el km quedaría correlacionado con la primera tirada de la firma (sección 7, §7.4). Un día usa `stageIndex` 1 (y no `arch|raceId` a secas, como escribe la sección 5, §5.7: la pasada de coherencia la alinea con esta lista, igual que la 8 y la 10). Las tres funciones de temporada se memoizan (sección 14) y son la ÚNICA puerta por la que `calendarRun.ts`, `callups.ts` y `raceContext.ts` leen una etapa no congelada (decisión 23); `stagesForSeason(id, 0)` devuelve por referencia las etapas del propio `SEASON_CALENDAR` (sección 10 lo exige con `toBe`).
...
// … V2 a V10 y V15 con la misma firma (solo V10 lee kmObjetivo)
...
// RouteStats es un `import type` de '../../sim/routeCensus.js': se borra al compilar, y veto.ts no importa ningún valor de sim/
...
// packages/engine/src/routes/stageKind.ts   (pasos 3 y 5: se añade `export` sin tocar cuerpos; paso 8: lo de la sección 11, §11.5 regla 2)
...
export function raceDePrueba(profile: StageProfile, kind: StageKind = 'reina'): CalendarRace   // una etapa, routeSource 'generado', sin arch: tests del censo
...
// = { raceId, stageIndex, season: BASE_SEASON, km, role, terrain: 'mountain', geo: ZONAS[q.geo], raceClass, format,
//     routeSource: 'generado', fixed: { skeleton: q.skeleton.id } }
...
El generador viejo vive durante el paso 9 en UN fichero, `sim/legacy/profileGenLegacy.ts`, con `legacyCalendar()`; el `sim/legacy/calendarLegacy.ts` que este bloque nombraba antes no existe (secciones 13 y 15). Para construirlo sin copiar 2.700 líneas de tablas, `calendar.ts` exporta en el paso 8 lo que hoy es privado (§3.11). Todo `sim/legacy/` se borra en el mismo cambio que cierra «vN §2» (decisión 29).
...
export const NATIONAL_CHAMPIONSHIPS: CalendarRace[]   // paso 8, ídem (hoy l. 379 en HEAD)
export function auto(segments: Segment[]): StageProfile   // paso 8, ídem (hoy l. 107 en HEAD)
// y editionGrandTour (hoy l. 247 en HEAD, firma sin cambios), paso 8, ídem; RaceRow (l. 381-398 en 8585ca2) pasa a exportarse con ellas
```
...
export function climb(rand: () => number, len: number, avg: number, opts: { gMax?: number } = {}): Segment   // hoy l. 72; gMax por defecto Infinity
...
Cuatro decisiones que las secciones dejaban abiertas. `huellaFNV` vive en `profileGen.ts`, junto a `hashInt`, y no en `realFingerprint.test.ts` como exportación de un test (sección 11, §11.3): `edition.test.ts` y `frozenSkeletons.test.ts` también la usan, y importar un fichero de test registraría sus `describe` en el fichero que lo importa. Es la misma función que `FrozenQueen.huellaFNV` guarda y que la sección 13 llama `fnv(profile)`. `GOLDEN` no es la misma huella: cifra el perfil ENTERO (pancartas incluidas) porque su trabajo es demostrar que el paso 1 no mueve nada, y la de lo real cifra solo `segments`, que es lo que §11.3 fija (las pancartas de una etapa real salen de los mismos rasgos por `bannersFromFeatures`, `featureProfile.ts` l. 299, así que no añaden información que sellar). `LegacySkeleton` deja fuera `id: SkeletonId` a propósito, para que el compilador impida que un esqueleto legado se cuele en el catálogo; por eso la tabla pareada no llama a `colocar`, que decide las bajadas por el `id` del esqueleto (§8.6 punto 1), sino a `colocarLegado`, que no decide nada porque `legacyMotivos` ya trae las bajadas y los enlaces del generador viejo. Y `EtapaSellada.label` se sella porque §11.3 punto 2 lo lista en la estructura, aunque el test escrito en §11.3 solo compara `kind`, `timeTrial` y `km`: la pasada de coherencia añade `label` a esa comparación.
...
`routes/arranque.test.ts` nace en el paso 1 y no en el 5 como escribe §15.7 («nace aquí con ese único `it`»), porque el `import type` de `calendar.js` que su primer `it` vigila aparece en el paso 1; la pasada de coherencia corrige §15.7.
