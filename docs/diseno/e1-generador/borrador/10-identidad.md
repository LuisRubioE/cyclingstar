...
edicion: {
  ...
  nivel: 1 as 0 | 1 | 2, // 0 arquitectura fija (solo cambia el dibujo); 1 jitter acotado; 2 rotación declarada (nivel efectivo 2 donde el esqueleto declare alternativas)
...
| 2 | además de 1, la opción de firma: `opcion = opcionDe(sk, raceId, season) = (hashInt(\`alt|${raceId}\`) + season) % (1 + sk.alternativas.length)` sobre `[canonico, ...alternativas]`, sin consumir ninguna tirada; la firma se instancia en `firma|raceId|i` dentro de los RANGOS de la opción (abajo) | todo lo demás igual que 1 |
...
export interface Alternativa {
  ...
  slots?: Record<number, Slot['params']>           // por índice de hueco con firma: true en sk.slots: sus rangos sustituyen a los del hueco
...
| `mot` | `mot\|${id}\|${season}\|${slot}\|${j}\|i${intento}` | instancia `j` del hueco `slot` | sí | sí | km, g, forma, estrellas del motivo no firma |
| `pos` | `pos\|${id}\|${season}\|i${intento}` | etapa | sí | sí | colocación por ventanas y `f` de cada bajada |
...
// MAX_TEMPORADAS_EN_MEMORIA = 8 con expulsión de la más antigua (sección 14, §14.5); cualquier otro cfg es de tests y del banco
...
// packages/engine/src/routes/calendar.ts: CalendarRace gana routeSource: RaceRouteSource (sección 3 §3.11 y sección 11 §11.2)
...
export function opcionDe(sk: Skeleton, raceId: string, season: number): number   // pura, sin dados: nivelEfectivo 2 ? (hashInt(`alt|${raceId}`) + season) % n : 0
...
export interface DiffInput {
  km: number                                    // Σ segment.km del perfil (kmDeStage): el km NO es un campo de Motif, por eso viaja aparte
...
describe('identidad entre ediciones', () => {
  ...
  it('las semillas son las literales de §10.4: vuelta, un día y edición real', () => {
    const vuelta = reqDe({ raceId: 'race-x', stageIndex: 3, season: 2, routeSource: 'generado' })
    ...
    const unDia = reqDe({ raceId: 'race-y', stageIndex: 1, season: 0, routeSource: 'generado' })
    expect(semillaDe('arch', unDia)).toBe('arch|race-y|1'); expect(semillaDe('ed', unDia)).toBe('ed|race-y|1|0')
    const edicion = reqDe({ raceId: 'race-italy', stageIndex: 5, season: 2, routeSource: 'edicion', editionKey: 'Foggia|Lucera|180' })
    ...
    for (const r of generadas) for (let s = 1; s <= 5; s++) {
      ...
      a.forEach((e, i) => {
        if (conCircuito(e)) {
          ...
          const q = kmDeStage(b[i]!.profile) / kmDeStage(e.profile)
  ...
  it('las etapas real no varían y las edicion varían solo el dibujo', () => {
    for (const r of SEASON_CALENDAR) stagesForSeason(r.id, 3).forEach((b, i) => {
      ...
      if (a.routeSource === 'edicion') {
        expect(kmDeStage(b.profile)).toBeCloseTo(kmDeStage(a.profile), 1)
  ...
  it('nivel 2: la opción rota por (hashInt(alt|raceId) + season) % n y dos carreras del mismo esqueleto no comparten firma', () => {
    const conAlt = generadas.filter((r) => skDe(r.stages[0]!).alternativas)
    expect(conAlt.length).toBeGreaterThan(0)
    for (const r of conAlt) {
      const sk = skDe(r.stages[0]!); const n = 1 + sk.alternativas!.length
      const opciones = Array.from({ length: n }, (_, s) => opcionDe(sk, r.id, s))
      expect([...opciones].sort()).toEqual(Array.from({ length: n }, (_, k) => k))          // en n temporadas seguidas pasa por todas una vez
      expect(opcionDe(sk, r.id, 0)).toBe((hashInt(`alt|${r.id}`) + 0) % n)
      for (let s = 0; s < n; s++) {
        const meta = stagesForSeason(r.id, s)[0]!.arch!.motivos.at(-1)!
        const op = opcionDe(sk, r.id, s)
        expect(meta.meta).toBe(op === 0 ? sk.meta : (sk.alternativas![op - 1]!.meta ?? sk.meta))
        expect(firmaDe(stagesForSeason(r.id, s + n)[0]!.arch!.motivos)).toEqual(firmaDe(stagesForSeason(r.id, s)[0]!.arch!.motivos))
      }
    }
    for (const [id, sk] of Object.entries(SKELETONS) as [SkeletonId, Skeleton][]) {
      const etapas = generadas.flatMap((r) => r.stages.filter((st) => st.arch!.skeleton === id))
      if (etapas.length < 2) continue
      const firmas = etapas.map((st) => JSON.stringify(firmaDe(st.arch!.motivos)))
      expect(new Set(firmas).size).toBeGreaterThanOrEqual(Math.ceil(etapas.length * 0.98))   // ≤ 2 % de coincidencias por redondeo al 0,1
    }
  })
