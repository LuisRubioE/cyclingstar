...
import type { Relieve, Altitud } from './routes/grammar/geo'
...
export const ARCH = {
  ...
  veto: {
    ...
    llana: { dPlusMax: 1800, cotaKm: 2.5, cotaG: 5, ventanaKm: 15 },
    calendario: { muroMin: 0.01, puncheurMin: 0.08 },
  },
  pancarta: { cimaMinKm: 1.5 },
  edicion: { activa: true, nivel: 1, kmJitter: 0.06, vueltasJitter: 0.5, motivoNuevo: 0.35 } as EdicionCfg,
  km: {
    porClase: { /* tabla 12.7 */ } as Record<Exclude<RaceClass, 'NC'>, Record<PapelKm, MinRango>> & { NC: { ruta: MinRango; sub23: MinRango } },
    maxPorClase: { WT: 260, Pro: 240, '1': 200, '2': 180, NC: 260 } as Record<RaceClass, number>,
  },
  pesoPorClase: { /* tabla 12.7 */ } as Partial<Record<SkeletonId, Partial<Record<RaceClass, number>>>>,
  pesosComposicion: { /* tabla 12.8 */ } as Record<Relieve, Partial<Record<StageRole, number>>>,
  bloques: { gv: { descansos: [9, 15] as Rango, reina: [15, 20] as Rango, primeraSemanaFinalesAlto: 1, maxAltaMontana: 7, minLlanasEntreBloques: 2 } },
...
| `motivo.cota.g` | [4; 8] (8 excluido: `between` es semiabierto) | La pendiente media de una cota. El techo es `STAGE.wallMinGradient` 8 (`constants.ts` l. 1685): al 8 % o más una subida se lee como muro, y por eso `muro.g` empieza donde `cota.g` termina. No es una regla del motor (`isWall`, `stage/sample.ts` l. 146-154, solo cambia MON por COL si además la subida mide ≤ `wallMaxKm` 2,5, con lo que una cota de 5 km al 7,5 % es MON de todos modos): es la frontera con la que un aficionado, `stageKindOf` (que no lee la pendiente para `kind`, `stageKind.ts` l. 60-64) y esta gramática separan cota y muro. Con techo 7, como estaba en §B.3, desaparecían las cotas al 7-8 % que firman `cantabrico` y `ardenas` (Arrate 5 × 7,4, mapa 07 §3 fila 3.8) y toda cota de media tenía una sola pendiente posible; `GeoSignature.cota.g` estrecha por zona (sección 6) | hoy [4,5; 6,5] (l. 248); mapa 07 §3 filas 3.2 y 3.8 |
...
| `motivo.muro.km` | [0,4; 2,5] | Muro: techo 2,5 = `STAGE.wallMaxKm` (`constants.ts` l. 1684), que es hasta donde `isWall` (`stage/sample.ts` l. 146-154: subida total ≤ 2,5 km con algún tramo ≥ 8 %) corre con COL en vez de MON; suelo 0,4 = `STAGE.finishClimbMinKm` (l. 4452), por debajo el motor no lo lee como cota. §B.3 ponía el techo en `WALL_MAX_KM` 3 (`stageKind.ts` l. 60, frontera clásica/media) y eso dejaba la franja [2,5; 3,0] con dos lecturas (muro para el clasificador, puerto corto para la ley de velocidad); se cierra con el umbral del motor, que es el más estrecho, y `meta.muro.km` [0,5; 2,2] queda dentro. Sacrificio: un muro vasco de 3 a 4 km al 12 % (mapa 07 fila 3.8) no existe en la gramática, ni como muro (> 2,5) ni como cota (g ≥ 8); se anota en la sección 17 | Kwaremont 2,2, Paterberg 0,36 (mapa 07 §1.3); hoy [1; 2,5] (l. 477) |
...
| Constante | Valor | Intención | En qué se apoya |
| --- | --- | --- | --- |
| `reina.dPlusIncluyeRelleno` | `true` | `Skeleton.dPlus` es el desnivel TOTAL, relleno incluido, porque es lo que `calendarQueens::desnivelDe` (l. 54-59) mide sumando bloques `subida`. Hoy el objetivo se persigue solo con los puertos (`dPlusOf`, `profileGen.ts` l. 325; `dPlusBase` l. 373) y el relleno añade de media 1.017 m sobre 2.840 en una reina de 175 km, con lo que `BANDAS_DESNIVEL` se lee sobre otra cifra que la que el generador persiguió | mapa 01 §1 y §2.5; el censo imprime el delta `dPlus` contra `dPlusBloques` (esperado < 5 %) |
...
| `veto.segmentoMinKm` | 0,5 | V10: ningún segmento por debajo. Es el suelo que `split` impone a cada trozo (`profileGen.ts` l. 52-65, `Math.max(0.5, …)` l. 58) y el umbral con que `rolling` descarta huecos (l. 101). El test de hoy (`calendar.test.ts` l. 92-108) solo exige `seg.km > 0` | sección 9 §9.1 |
...
| `veto.llana` | { dPlusMax: 1.800, cotaKm: 2,5, cotaG: 5, ventanaKm: 15 } | V9: una llana tiene `dPlusDe ≤ 1.800` y ninguna racha de ≥ 2,5 km a ≥ 5 % que termine en los últimos 15 km (una llana de 2.500 m es media; hoy el relleno solo ya da de 661 a 1.413 m) | mapa 07 §4.4 regla 10; mapa 01 §1; sección 9 |
...
Consecuencias que el censo mide: las 142 carreras de un día con el 210 por defecto de `RaceRow` (`row.km ?? 210`, `calendar.ts` l. 929, mapa 02) pasan a sortear su km con `firma|raceId` dentro de la fila `unDia` de su clase desde la temporada 0, y las 36 filas con `km` explícito lo conservan (Sanremo 294 va por fila). En las ETAPAS DE VUELTA de una .2 desaparecen las de 165 a 195 km (hoy `kmFlat` no distingue clase, l. 1331): sus columnas terminan en 150, 150, 155 y 120. Un día .2 llega a 180 por diseño, así que la banda `km.clase` de `ROUTE_CENSUS_TARGETS` (sección 13 §13.3) se escribe en dos filas y no en una: «etapas de vuelta .2: p90 ≤ 150» (uniforme en [110; 150] tiene p90 146) y «un día .2: p90 ≤ 176, máximo 180» (uniforme en [140; 180] tiene p90 176); un «p90 de toda la clase .2 ≤ 170» dependería de cuántas .2 son de un día frente a etapas de vuelta, que nadie ha contado, y podría nacer rojo por aritmética.
...
Lo que cambia respecto de hoy y el censo mide: `mixWeights.mountain` da un 40 % de reinas (l. 1318) y una vuelta de 21 generada en `montana`/`alta` pasa a 4-6 etapas de alta montaña repartidas en cuatro formas, que es lo que llevan Tour y Giro (mapa 07 §2.1). Las cuatro garantías de `mixRoles` (`calendar.ts` l. 471 y siguientes: crono, última decisiva, `selectiveMinFraction`, `uphillFinishMinStages`) siguen leyendo `ROUTE` y se aplican encima (sección 7).
...
Se retira en el paso 8 del plan, en el mismo cambio que borra los ocho `xxxSegments`, y con una nota en `docs/balance.md` cuyo nombre de esqueleto es «v61 · El generador es una gramática». Como al implementar ya existe una v61 (la tabla de `balance.md` l. 11171 y l. 11241 la citan, l. 11337 es la v62 y la última nota es la v86, l. 16332), la nota toma el siguiente número libre, que es el mismo al que sube `ENGINE_VERSION` (12.13): «v87 · El generador es una gramática» si nadie ha movido la versión al empezar el paso 8; todas las referencias «v61 §n» de este documento se leen con ese número. La tabla «constantes retiradas y sus sustitutas» va en su §1, con el valor de hoy al lado, como pide `Claude.md` l. 12. Las claves se citan por nombre; sus líneas en `constants.ts` al corregir son `queenDplusRange` 1258, `queenHighDplusShare` 1260, `queenLowDplusRange` 1261, `queenFinalMix` 1276, `grandTourStages` 1307, `grandTourLastDecisiveFactor` 1308, `mixWeights` 1315-1319, `kmFlat..kmSummit` 1331-1334.
...
| `grandTourStages`, `grandTourLastDecisiveFactor` | 15, 0,4 | el paso 2 de `mixRoles` (`calendar.ts` l. 490-491: `factor = n >= grandTourStages ? grandTourLastDecisiveFactor : 1`) | `TOUR_SKELETONS.vu_gran_vuelta.ultima` = { llana 0,85; cri 0,15 } (sección 7 §7.2): en una gran vuelta generada el paso 2 se sustituye por esa tirada y la constante se quedaba sin lector (las tres grandes vueltas reales no pasan por `mixRoles`; una constante viva sin uso incumple `Claude.md` l. 12). §B.1 la listaba entre lo conservado; se corrige aquí y la sección 7 quita `× grandTourLastDecisiveFactor si n ≥ 15` de su descripción del paso 2, que en E1 solo corre con `n ≤ 14`. Ningún test de `calendar.test.ts` la cita (la única mención de gran vuelta, l. 70-72, es por `format`) |
...
Se conservan tal cual, con su línea y su lector: `ittMinStages` 3, `ittChanceShort` 0,6, `ittChanceWeek` 0,9, `ittWeekStages` 6, `ittAlwaysFlatStages` 4, `ittEarlierChance` 0,35, `ittSecondStages` 15, `ittSecondPosition` 0,35 (l. 1280-1294, `mixRoles`); `ittKmMin` 14, `ittKmRange` 12, `ittLongStages` 10, `ittLongKmMin` 26, `ittLongKmRange` 18 (l. 1296-1300, `mixKm`, que pasa a `kmDe` para `cri`); `lastDecisiveChance` {0,3; 0,55; 0,85} y `lastSummitShare` {0; 0,35; 0,8} (l. 1306 y l. 1311, última etapa de `vu_corta`, `vu_semana` y `vu_larga`); `selectiveMinFraction` {0,35; 0,55; 0,7} y `uphillFinishMinStages` 4 (l. 1323-1326, garantías); `lastStageKmFactor` 0,85 (l. 1336). Son composición del calendario, no perfil (mapa 01 §3), y `composeTour` los lee por el mismo nombre para que `calendar.test.ts` l. 184-277 siga en verde sin re-sellar (sección 7). Las claves de `lastDecisiveChance` y `lastSummitShare` siguen indexadas por `MixTerrain` (`flat`/`hilly`/`mountain`), que en E1 sobrevive solo como el `terrain` de la fila de la carrera (`RouteTerrain` de `featureProfile.ts` l. 21) y se usa como sesgo, nunca como orden (sección 3).
...
| `STAGE.wallMaxKm` / `wallMinGradient` | 2,5 / 8 | `constants.ts` l. 1684-1685 | Ley de velocidad (COL frente a MON, `isWall` en `stage/sample.ts` l. 146-154); `ARCH.motivo.muro.km[1]` 2,5, `.gMin` 8 y `.g[0]` 8 los referencian, y `ARCH.motivo.cota.g[1]` 8 se apoya en el segundo |
...
Vive en `grammar/motifs.test.ts` (bloque «`ARCH` es coherente con `routes/` y `STAGE`») y corre en `test:rapido`; falla si alguien separa una referencia de su fuente, abre un rango por encima de un umbral del clasificador o del motor, o vuelve a cerrar el ciclo de importación:
...
describe('ARCH es coherente con routes/ y STAGE', () => {
  ...
  it('muro, cota y repecho respetan los umbrales del clasificador y del motor', () => {
    expect(ARCH.motivo.muro.km[1]).toBe(STAGE.wallMaxKm)                 // 2,5: hasta donde isWall corre con COL
