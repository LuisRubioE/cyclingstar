...
Las líneas de código que cita esta sección están contadas sobre el HEAD `8553486` (el árbol del día de la corrección), no sobre el `8585ca2` de los mapas: `calendar.ts` ganó 14 líneas desde entonces (`doubleAfter`, l. 79-92) y `calendarRun.ts` unas 16, así que las citas de los mapas para esos dos ficheros no valen aquí. Quien implemente recita cada línea sobre su propio HEAD antes de tocarla; los nombres de función son la referencia estable, la línea es una ayuda.
...
| `stagesFromEdition` sin rasgos (l. 237, rama `oneDaySpec`) | edición en `RACE_EDITIONS`, `features?.[i]` nulo | `edicion` | `generateStage` con `km` de la edición como contrato: hoy al kilómetro entero (`calendar.test.ts` l. 140-153, `expect(Math.round(km)).toBe(edition.stages[i].km)`), y desde el paso 8 al 0,1 (`toBeCloseTo(edition.stages[i].km, 1)`), porque `normalizeEnlaces` cuadra al 0,1 y las reales ya cuadran por `normalizeTotal` | uno de ETAPA por `EditionTerrain`: `flat → et_llana`, `hilly → et_media_*`, `mountain → et_reina_*`, `itt → et_crono`; `cobbles → ud_adoquin_ligero` (única excepción, tres etapas, abajo; tabla completa en la sección 5) | `${raceId}\|e${i}\|${editionKey}` con `editionKey = ${from}\|${to}\|${km}`; `season` solo en `dib` (decisión de abajo) |
| un día de tabla sin rasgos (l. 925-933) | sin edición, `row.stages ≤ 1`, sin rasgos | `generado` | `generateStage` con `role: 'un_dia'` y `km` de `ARCH.km.porClase` salvo fila con `km` explícito (`row.km ?? 210`, l. 929) | los 15 de un día por `regionOf(raceId, 1, country)` | `arch\|raceId`, `firma\|raceId`, `ed\|raceId\|season` |
| vuelta de tabla (l. 934-938) | sin edición, `row.stages ≥ 2` | `generado` | `composeTour` y `generateStage` por etapa | los 17 de etapa por `itinerarioDe` | ídem, por etapa |
  ...
  it('las tres grandes vueltas conservan estructura, y perfil en sus etapas reales', () => {
    for (const [id, gv] of Object.entries(GRANDES_VUELTAS)) {
      ...
      race.stages.forEach((st, i) => {
        const e = gv.etapas[i]!
        expect([st.kind, st.timeTrial ?? false, profileKm(st.profile)]).toEqual([e.kind, e.timeTrial, e.km])
...
Hoy `race_routes.route_source` recibe `'generado'` fijo (`db/raceRoutes.ts` l. 48-51: «el calendario no declara todavía de dónde viene cada recorrido»), el tipo `RouteSource` de l. 29 tiene dos valores, y la columna es `text` con default (`schema.ts` l. 541). Por eso los tres valores NO exigen migración por el tipo: se cambia el tipo TypeScript y el valor escrito. Lo que sí trae migración son las columnas `kind`, `label` y `time_trial` de la decisión 23 (nullable, porque las filas congeladas antes del paso 10 no las tienen), que viajan en la misma fila: se añaden a `raceRoutes` en `packages/db/src/schema.ts` l. 530-544 y la migración se GENERA con `drizzle-kit generate` en `packages/db/drizzle/` (hoy la última es `0040_ordenes_del_paso_17a.sql`; la nueva toma el siguiente número libre y el nombre que dé la herramienta, con su snapshot en `drizzle/meta/`), porque `Claude.md` l. 14 manda «Migraciones solo con drizzle-kit; nunca SQL manual en producción». El directorio `packages/db/migrations/` de §B.1 no existe.
...
| `viento` | 2, 3 | «llano abierto» | describe la carretera; el abanico sigue saliendo de `rng('viento')` en cualquier km de `llano` (mapa 03 §5.1) y no de este metadato |
...
it('sobre el calendario entero solo cambia la etiqueta, nunca el tipo de etapa', () => {
  ...
  expect(cambianReales).toBe(0)      // cifra medida en el paso 8 y escrita aquí con su causa (balance.md v61 §1);
...
- Una reina `cima_cerca` con 3 km de valle sale `reina / Summit finish` en la ficha Y en `stageHistory`, que es la corrección de §11.5 (regla 2 en el clasificador, regla 3 en `labelDe` y en `calendarStageSpec`); hoy sale `Mountains` en una y `Summit finish` en la otra.
