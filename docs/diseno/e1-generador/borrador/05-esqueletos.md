...
export const NC_RUTA_CLASICA: Motif[] = [E(15), K(16, 12, [M(1.0, 9), M(0.5, 10)], [4, 7.5], true), META('esprint', 3)]   // 210 km; último muro a 6,0
...
SKELETONS.ud_montana.alternativas = [{
  ...
  canonico: [E(116), P(9.4, 6.6), D(10), E(20), P(11.6, 8, true), D(10), E(20), P(9.2, 7.3), D(10), E(10), C(3.0, 7), D(3.8), E(10),
    META('descenso_meta', 7.0, { km: 1.3, g: 7 })],                                                                               // 250 km
}]
SKELETONS.et_reina_alto_largo.alternativas = [{
  nombre: 'Angliru',
  metaParams: { kmRango: [12, 13], gRango: [8.5, 9.5] },          // ∩ ARCH.meta.altoLargo.g [6; 9]: en la práctica [8,5; 9]
  // 12,5 km al 9 % (real 9,8; techo ARCH.meta.altoLargo.g)
  canonico: [E(52.5), P(12, 7), D(10), E(20), P(14, 7), D(10), E(18), P(10, 7.5), D(10), E(6.0), META('alto_largo', 12.5, { km: 12.5, g: 9 })],   // 175 km
}, {
  nombre: 'Lagos',
  metaParams: { kmRango: [11.5, 12.8], gRango: [6.8, 7.6] },
  // Lagos de Covadonga: 12,2 km al 7,2 %
  canonico: [E(52.5), P(12, 7), D(10), E(20), P(14, 7), D(10), E(18), P(10, 7.5), D(10), E(6.3), META('alto_largo', 12.2, { km: 12.2, g: 7.2 })], // 175 km
}]
```
...
export function degradarPapel(role: StageRole): StageRole | null { return ESCALON_ROLE[role] }
...
type Peticion = Pick<StageRequest, 'role' | 'terrain' | 'geo' | 'raceClass' | 'format' | 'km' | 'routeSource'>
...
const peso = (id: SkeletonId, terrain: RouteTerrain | null, req: Peticion): number =>
  SKELETONS[id].pesoBase * (terrain === null ? 1 : SESGO_TERRENO[terrain][id]!) * ARCH.pesoPorClase[id][req.raceClass] * (req.geo.pesos[id] ?? 1)
...
describe('plantilla canónica', () => {
  it.each(Object.values(SKELETONS))('$id: pasa validateMotif y los vetos, y da su kind y su finalKind sin RNG', (sk) => {
    ...
    for (const { canonico: alt } of sk.alternativas ?? []) {                    // Alternativa (§10.3): se valida su plantilla literal
...
describe('candidatos y pesos', () => {
  ...
  it('hilly en una zona sin muro: tres candidatos desde .1, y en .2 donde la cota baja de 2,9 km', () => {
    const sinMuro: GeoZone[] = ['alpes', 'pirineos', 'dolomitas', 'meseta', 'balcanes', 'anatolia', 'andes', 'cono_sur', 'golfo', 'africa_llana']
    const ids = (zona: GeoZone, raceClass: RaceClass) => candidatos({ role: 'un_dia', terrain: 'hilly', geo: ZONAS[zona], raceClass, format: 'un-dia', km: 160, routeSource: 'generado' }).map((c) => c.id)
    expect(Object.values(ZONAS).filter((g) => g.muro === null).map((g) => g.zona).sort()).toEqual([...sinMuro].sort())
    for (const zona of sinMuro) {
      expect(ids(zona, '1')).toEqual(expect.arrayContaining(['ud_montana_media', 'ud_repecho', 'ud_esprint_capi']))
  ...
  it('un nacional solo sale de nc_ruta o nc_crono, y ninguna carrera de equipos sale de nc_ruta', () => {
    ...
    const chrono = RACE_ROWS.find((r) => r.id === 'race-chrono')!                                       // .1, itt, 45 km (calendar.ts l. 3365-3374)
