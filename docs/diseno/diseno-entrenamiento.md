...
- Las citas del dueño van entre «» **y solo esas**. Lo que él ya dictó no se discute: «hay que ser
  ...
  (`mapa-requisitos-duenio.md:227`); «de una carrera puedes aprender más que de un entrenamiento, e
  incluso variará según el nivel de la carrera» (`mapa-requisitos-duenio.md:229`); el entrenador bot
  «razonable, nunca óptimo»; «que no acaben todos siendo Pogačar»; bots con cinco estrellas
  «claramente menos del 15 %» (`mapa-requisitos-duenio.md:231`); el humano «empieza con 18 años… con
  stats casi a cero, sin equipo»; «no quiero una academia junior de bots» (`mapa-spec.md:219`); la
...
Esto además toca una deuda abierta del corpus que conviene cerrar de paso: `mapa-requisitos-duenio.md:278`
...
`mapa-requisitos-duenio.md:227`, cuyo defecto medido era «79,6 % del pelotón congelado en temporada
...
- **Valores iniciales**: `mu_valor = 15 + 9·bias` con sd 3 (con los tres escalones reproduce
  ...
  `mapa-entrenamiento-atributos.md:158-163`). TAC `U(12, 16)`. Don global como hoy (`max(techo) < 82`
...
entrenamiento y carreras» (también recogido en `mapa-bancos.md:655`), y D0-D5 solo deciden
...
- **`mHealth('molestias') = 0,96` en carrera también hay que medirlo antes de meterlo.** Entra en
  ...
  etapas (deuda anotada en `mapa-requisitos-duenio.md:314`, §14.38). Sin medirlo sería un −4 %
...
Frontera 2 es que los bancos valen porque corren «con forma de producción» (`mapa-bancos.md` §1.3);
