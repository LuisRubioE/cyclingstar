...
- País preseleccionado por geolocalización de IP: la resuelve SIEMPRE el servidor (`GET /api/geo/country`). Primero mira las cabeceras que ponga la red de delante (`CF-IPCountry` y equivalentes de Vercel/Fastly), gratis e instantáneas; si no hay ninguna —hoy Railway no pone ninguna—, consulta con la IP del cliente un servicio público de IP a país (ipapi.co, con ipinfo.io e ipwho.is de reserva) y cachea el resultado en memoria. Nunca se consulta desde el navegador: nuestra propia CSP (`connect-src 'self'`) lo prohíbe. Alternativa autónoma futura, base GeoLite2 country consultada en local (mmdb, gratuita con atribución). La IP se usa al vuelo y no se persiste. El país es siempre editable (VPN, expatriados) y, confirmado, alimenta campeonato nacional y Mundial.
